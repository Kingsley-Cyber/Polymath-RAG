"""ChunkKind taxonomy — ported from polymath v3.3 section_classifier.py
(Docling-lineage structure layer, MIT, same project). Classifies a chunk by
heading path so the LLM neighborhood builder can skip structural noise
(TOC/bibliography/index/front/back matter): the dominant per-chunk token
cost for zero information gain."""
