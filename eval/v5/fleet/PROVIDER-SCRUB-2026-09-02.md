...
| qwen/qwen3.7-flash @ OpenRouter | model@host | **CANARY PASS (70 s) — wireable, reasoning OFF mandatory** | quick grade B (0.787) and production canary PASS with reasoning_effort none: 8/8 extraction at 5.3 s mean and 103–130 tok/s, 0 limiter wait, facts 15.4/1Kw (between gemini-lite 12.1 and ministral-14b 23.7), enrichment 8/8 gist 1.00. Single provider (Alibaba), $0.03/$0.13 per M. As-is (thinking on) it is unusable: 2,500 reasoning tokens, empty content. Receipt run still owed before a lane is pinned |
