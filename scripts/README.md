...
| `scripts/trace_stalls.py` | control | stalled units (stage tickets / runs / summary jobs) older than control.stall_threshold_s | STALL-TRACER-V1 operator view: control-plane heartbeat age, stored open `stall_traces`, and a live READ-ONLY collect (rolled back) with the tracer's diagnosis per unit; `--live`, `--threshold N` | read-only |
