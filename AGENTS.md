...
000. **CURRENT operating state (2026-09-13, register 11.257) — `docs/wiki/plans/CONTINUITY-REPORT.md` is the single living bootstrap and supersedes every dated handoff below.** The Groq Parent-MAP forensic audit is DONE (11.253); the §19 / U-2 hold was LIFTED by owner directive and the cinema pMAP backfill COMPLETED (11.255: 3,810 → 0 unresolved parents); the control tick was repaired (11.256). Active owner goal: **COGNITIVE-ADAPTER-TRAIL-E2E-V1** — `docs/wiki/plans/COGNITIVE-ADAPTER-TRAIL-E2E-V1-START-HERE.md` on branch `handoff/unified-adapter-trail-e2e` (Polymath = composition root, TrailSignal = required internal subgraph). Items 00 and 0a below are HISTORICAL: the hold text in 0a no longer applies.
00. **HISTORICAL (2026-09-09; item 000 supersedes it) — `docs/wiki/reports/2026-09-09/START-HERE.md`.** (Corrected 2026-09-24, register 11.463: `architecture/evidence-first-v5` is archived as tag `archive/architecture/evidence-first-v5`; `RAG_PIPELINE_FINISH_PLAN.md` exists at the repository root.) Control-plane repair LANDED (register 11.185; branch `architecture/evidence-first-v5` @ `f1112ce`, 4 commits AHEAD of origin, unpushed). Cinema pMAP backfill STOPPED; the only remaining gate is a **bounded, owner-authorized Groq live probe** (quota topology) → benchmark → canary. Read the numbered pack `01_…`–`09_…` in that folder. Execution authority = `RETRIEVAL-MIGRATION-DEPENDENCY-V1.md` (S-slices) + `FINAL-RETRIEVAL-ROUTING-SYNTHESIS-V1.md` (P-phases) + `PLAN-AUTHORITY-REGISTER.md`; note: the 2026-09-09 claim that `RAG_PIPELINE_FINISH_PLAN.md` does not exist was wrong (it is at the repository root).
0a. **`docs/wiki/reports/2026-09-08/START-HERE.md` — the Groq forensic-audit handoff** (the operating state in item 00 supersedes it; still valid for the audit's targets/detail). Original note (FORENSIC HOLD 2026-09-08/09).** `CINEMA PARENT-MAP BACKFILL = STOPPED`; the prior "Groq RPD exhausted" conclusion is DISPUTED. The next session's principal task is the **Groq Parent-MAP forensic audit** (`GROQ-FORENSIC-AUDIT.md`), NOT a backfill and NOT a code patch. Do not resume backfill on a quota reset; do not spend provider quota to gather evidence; architecture frozen (ParentSkeleton → plaintext MAP DSL → deterministic `map_compiler` → maps → projection; no JSON mode; chunker untouched). This folder also carries CONTINUATION_REPORT / BE_AWARE / UNFINISHED_WORK / DEPENDENCY_MAP.
...
6. `docs/wiki/plans/DOCUMENT-SEMANTIC-INDEX-V1-PLAN.md` — the **next-phase plan of record** (document semantic index + parent map + vocabulary bridge; slices S0–S16), with `docs/wiki/plans/DOCUMENT-SEMANTIC-INDEX-V1-START-HERE.md` as its coding-agent hook. Current slice: **S0-S4 + corrective checkpoint + Groq router core + LIVE map-contract canary + ParentSkeleton v2 (lead_excerpt) done (11.129-11.137, HEAD 0962f83); the parent-map contract is proven live (22/22 & 11/11 mapped, injection resisted) and headingless parents now carry opening framing. S5 profile vNext + fingerprint next. Groq live wiring + reindex canary gated (fleet config + provider spend). See docs/wiki/reports/2026-09-07T1828/ for the full continuation.**
