"""Reasoning modes — ported VERBATIM from polymath v3.3
(backend/services/reasoning.py, Phase 15) at the owner's request
(2026-08-27): the system-prompt reasoning layer.

Prompt-only port: every template prepends to the generation prompt.
v3.3's two pipeline modes (atomic, self_correct) act as prompt-only
templates here — v4's retrieval pipeline is contract-frozen and is not
mutated by a chat-layer flag.
"""
...
#: Curated modes for the UI dropdown (v3.3 primary dropdown set).
CURATED_MODES = [m.value for m in ReasoningMode]


def apply_reasoning(prompt: str, mode: str | None = "none",
                    blend: list[str] | None = None) -> str:
    """Prepend reasoning template(s) to `prompt` (verbatim v3.3
    semantics): blend parts first, then the main mode, then the prompt."""
    parts: list[str] = []
    if blend:
        for key in blend:
