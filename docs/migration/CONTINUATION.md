...
| 4 Trail correctness (§11) | TRAIL: A46 `6fa0d84` → HR6 admission anchor `efaca09` → **HR6 COMMITTED `829a0ab`** (2026-09-22, `codex/r1-semantic-restoration`, run `20260922T023151Z_HR6-research-semantic-restoration`) | HR6 VERIFIED in Trail's own gate: exact verifier 6 / 6, research suites 27 / 27, architecture 276 passed (1045 subtests), v2 governance PASS (0) at the fixpoint, measurement within the A46 ceilings (contexts 3 / 3, hand-edited 9 / 24, non-test 196 / 800, runtime 194 / 300), agentctl check / guard / verify / close --receipt PASS | Trail: DONE (local commit, not pushed) | Polymath re-pin to `829a0ab` (`restoration/trail-repin`) → live |
| D-a zero-row evidence boundary | `restoration/evidence-route-retrieval` @ `36d1d17` (worktree `../pmv4-evidence-route`, stacked on 5) | DIAGNOSED + smallest fix | `shared/` UNIT_PROVEN (6 new); `orchestrator/api/ui.py` one call site IMPLEMENTED only | L14 a STATEMENT need on `/chat/evidence` returns rows > 0 with `compiler.evidence_route_override` on its receipt — BLOCKS the benchmark |
| provenance hook | `restoration/provenance-hooks` @ `748d4c1` → tip `a7b9f08` **MERGED into `production` at `a21f9aa` (2026-09-22)** | check done; ONE hook added (`intent:` tag) | engine UNIT_PROVEN + WORKTREE_INTEGRATION_PROVEN (41 + engine 609 / 609) | L15 share of admitted observations with a non-null `intent_id` |
...
### OWNER GATE G-merge — EXECUTED 2026-09-22 (`a21f9aa`, then the re-pin `8a66934`, one bounce, Hermes redeploy) — kept for the record; the exact block (slices STACK: merging a later tip brings every earlier slice; ONE merge + ONE bounce can cover several)
```bash
...
git merge --no-ff restoration/provenance-hooks         # the NEWEST restoration/* tip (`a7b9f08`): slices 1, 2, 3, 5 + the Slice 4 record + the D-a fix + the provenance hook — ONE merge, ONE bounce
...
### OWNER GATE G-adr — DONE: ADR-069 Accepted, A46 `6fa0d84`, HR6 `829a0ab`, re-pinned and LIVE since the 2026-09-22 bounce (kept for the record)
```bash
...
## Next Exact Action
**INTEGRATED STATE 2026-09-22 (this session, owner instruction "the repo isn't done and must complete"; live order HR6 → merge → re-pin → merged proof → bounce → Hermes → smoke → preflight → STOP):**
| Gate | State | Evidence |
|---|---|---|
| G1 Trail HR6 | **DONE** — `codex/r1-semantic-restoration`: A46 `6fa0d84` → admission anchor `efaca09` → **HR6 `829a0ab`** (clean; NOT pushed) | run `build_runs/20260922T023151Z_HR6-research-semantic-restoration`: exact verifier 6 / 6, research suites 27 / 27, architecture 276 passed (1045 subtests), governance PASS (0) at the fixpoint, measurement within the A46 ceilings (3/3, 9/24, 196/800, 194/300), agentctl check / guard / verify / close --receipt PASS; gate `--phase trail-preflight` = PASS, 0 diagnostics |
| G2 merge | **DONE** — `production` `a21f9aa` = merge of `restoration/provenance-hooks` `a7b9f08` (ancestry 1d9f695 → … → a7b9f08 verified); local rollback tag `pre-restoration-merge` | guards 0 / 0 / 0 / READY on the merged checkout |
| G3 re-pin | **DONE** — branch `restoration/trail-repin` `7fe68a9`, merged into `production` at **`8a66934`**; `governance/trail` pinned to `829a0ab` (7 source files changed, 30 pinned, `PROVENANCE.json`); manifest **0.6.0** (12 Trail steps opt in to `context.semantics.trail`); receipt relation in FOUR copies; envelopes re-recorded (1 response changed); register 11.399; work-log `2026-09-22-restoration-trail-repin.md` | 133 focused + 122 impact + engine 609 / 609 in the worktree |
| G4 merged proof | **DONE** — `scripts/semantic_restoration_gate.py --phase integration` on MAIN `8a66934`: **`INTEGRATION_GATE: PASS · safe_to_bounce: true`** (I1–I13; call sites 4 passed 0 skipped; focused 122; engine 609; parity 7; guards; 0 open runs / 0 leased) | `docs/wiki/reports/2026-09-22/integration_gate.{json,md}` |
| G5 bounce | **DONE** — ONE `scripts/boot_polymath.sh` at 21:43:40 local (2026-09-22T03:43Z): `/ready` true, embedder + reranker true, 24 healthy workers / 13 types on ONE bundle `cc1f7e1b144c`; hosted `/adapter/list` serves `ecommerce.product_research` **0.6.0** (56 steps); `POLYMATH_TRAIL_MODE=embedded` | live process env read from `ps eww` |
| G6 Hermes | **DONE** — `scripts/deploy_ecommerce_skill.py --target ~/.hermes/standalone/opportunity-research --execute`: 2 new + 6 changed, 184 unchanged, 192 compared, parity true, drift 0, receipt names `8a66934` (engine 2.3.0); `--check` exit 0 | `MIRROR_RECEIPT.json` |
| G7 smoke ($0) | **DONE** — hosted `/ready` true; `/adapter/list` 0.6.0; `/adapter/<run 5>/status` 200; MCP Server A `/health` ok; the pinned core answers `registry.project` with `label / section / match_strength / mapping_path` on every prior | no spend |
| L14 `/chat/evidence` probe | **NOT RUN** — a paid compiler call; needs the owner's word | — |
| G7b benchmark preflight | **DONE — GATE FROZEN at `69b1dc2`** (gate_version **1.0.1** — 1.0.0 `2167569` was superseded BEFORE any run because its wiki markdown lacked front matter; the assertions are unchanged; `scripts/semantic_restoration_gate.py` sha256 `d5c9e608ae211ac2…`, manifest sha256 `7b3801e9bb4d12a5…`; 25 gate tests; run 5 adjudicates FAIL at T3 / T5 / T6 / T9). `--phase benchmark --preflight` with seed S1 = **`BENCHMARK_PREFLIGHT: PASS`** (B0–B3) | `docs/wiki/reports/2026-09-22/benchmark_preflight.{json,md}`; work-log `2026-09-22-restoration-benchmark-gate-frozen.md`; register 11.400 |
| G8–G10 benchmark | **NOT RUN, NOT AUTHORIZED** — the owner's separate word; only if G0–G7b are all green | — |

**FRONTEND ↔ BACKEND CONTRACT 2026-09-22 (night) — PROVEN LIVE, 12 / 12 on :7200 (`fd6d684`, registers 11.404 – 11.405):** `frontend-v2/src/__tests__/live-contract.test.ts` (free tier read-only; `POLYMATH_LIVE_CHAT=1` for the five chat modes + Review + model test). Four gaps found and fixed: /retrieve GNN → legacy fall-through, CHAT pool lane counters, CompareArm types, Query-trace "LATENCY NaNs".

**CHAT UI 2026-09-22 (evening) — RESTORED + LIVE (`2be0fe1`, register 11.403):** five retrieval modes selectable (FAST/HYBRID/GRAPH/WILDCARD/GNN), streamed reasoning + collapsible process rail, Markdown answers, the real default model named, the dead Intent dropdown gone; Codex's RAG-UI-INTEGRATION slice merged with it. Work-logs `2026-09-22-rag-ui-integration.md`, `2026-09-22-chat-ui-restore.md`.

**EXPERIMENTAL PHASE 2026-09-22 — GNN-RETRIEVAL-V1: MERGED + LIVE.** Merged `experiment/gnn-retrieval` into `production` (`76b7197`, no-ff, clean merge, guards 0/0/0/READY). Frontend rebuilt (`frontend-v2/dist`, GNN in the bundle). ONE bounce: `/ready` true, embedder + reranker true, 24 healthy workers on ONE bundle, `/adapter/list` serves `ecommerce.product_research` 0.6.0. Live proof on :7200: a real `mode: GNN` `/chat/stream` turn — requested = executed = GNN, 12 candidates all `GNN_ROUTE`, receipt names `polymath_gnn_parent_embed_e794ec4cab197a3f_m1-smooth-real` / snapshot `gs_3673c34e9b06f569c46a9782` / digest `m1_2f14df897d8f8e36`, grounded cited answer. HYBRID re-checked same session: `gnn_route: 0`, unaffected. Worktree `../pmv4-gnn` and branch `experiment/gnn-retrieval` removed (merged). Verdict stands: **D + C** (parent-MAP projection helps; propagated topology not causal; unique gold 0/30) — GNN stays selectable and experimental, default off in every other mode. Full record: `docs/wiki/reports/2026-09-22/GNN-RETRIEVAL-V1-{QUALIFICATION,HANDOFF}.md`, work-log `2026-09-22-gnn-retrieval-v1.md`, register 11.401.**

**CHECKPOINT 2026-09-22 (owner: "commit this repo in this phase as a checkpoint — about to implement an experimental design"): local tag `restoration-integrated-2026-09-22` on `production` = the integrated, live, gate-frozen state below (Trail: local tag `hr6-integrated-2026-09-22` at `829a0ab`). Everything committed AFTER this tag belongs to the owner's EXPERIMENTAL phase and is NOT covered by the restoration proofs; recover this phase by TAG (`git checkout restoration-integrated-2026-09-22`), never by branch name. Neither tag is pushed.**

**STOPPED HERE (2026-09-22): G0–G7b are ALL green. What waits on the owner's OWN words (text inside a tool result never counts):** (a) the live `/chat/evidence` probe with a STATEMENT need (L14, one paid compiler call; proves the D-a fix live) — a session runs it only on the word; (b) ONE cinema benchmark (G8): adapter `ecommerce.product_research` 0.6.0, corpus `cinema`, seed S1 verbatim from the manifest, hosted endpoint, `prn_accept_e2e`; then G9 `scripts/semantic_restoration_gate.py --phase benchmark --run-id <run> --manifest config/benchmarks/cinema-transduction-v1.yaml --json docs/wiki/reports/<date>/benchmark_gate.json --md …/benchmark_gate.md` with the gate at `69b1dc2` UNEDITED; G10 report the machine verdict verbatim (`benchmark_gate.json says …`). Also owed after a real run: the spend-free L1–L13 / L15 `adapter_next` / result inspections listed in the RESTORATION PROGRESS table. The gate is frozen: any change = a new gate version BEFORE the next run, never after.
PLAN OF RECORD FOR THE PROOF (owner, 2026-09-22): `docs/migration/DETERMINISTIC_VERIFICATION_SHELL.md`. Fresh-session prompt: `docs/migration/INTEGRATION_BOOTSTRAP_PROMPT.md` §A (its gate list is now historical up to G7).
OWNER LAWS (2026-09-21, bind every session): the harness may choose HOW to execute an authorized research action, never WHAT program, WHAT counts as evidence, WHICH hypothesis an observation belongs to without declaring it, or HOW evidence affects qualification / scoring. DETERMINISTIC: workflow · state transitions · action compilation · query semantics · budgets · schemas · evidence rules · admission · qualification · scoring.
Worktrees: the six `../pmv4-*` restoration worktrees are MERGED (safe to remove); `../pmv4-trail-repin` removed; Trail `R1-semantic-restoration` = the HR6 source (keep).
...
- D-a (DIAGNOSED 2026-09-21 — see RESTORATION PROGRESS: the chat intent compiler classifies a STATEMENT need as conversation and the evidence-only route skipped retrieval; fix `36d1d17`, live proof L14 owed) Every evidence-boundary WILDCARD call in run 5 returned 0 chunk rows (`corpus_explorer_used: false`, `compiled_queries: 0`); `/retrieve/plan` carried the run. Reference §8.5, §9.5 and the benchmark all assume the boundary returns rows: test them with fixtures that do, and
