...
| G7b benchmark preflight | see below | — |
| G8–G10 benchmark | **NOT RUN, NOT AUTHORIZED** — the owner's separate word; only if G0–G7b are all green | — |

NEXT: G7b — build the `benchmark` phase of the gate (T1–T11 per `DETERMINISTIC_VERIFICATION_SHELL.md` §2.3) + `config/benchmarks/cinema-transduction-v1.yaml` + fixture tests (run 5 read-only must FAIL at T5 / T6 / T9), commit, FREEZE (record the gate SHA here), then STOP and report.
