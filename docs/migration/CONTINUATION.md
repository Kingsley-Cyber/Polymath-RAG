...
## Next Exact Action
**PASTE `docs/migration/INTEGRATION_BOOTSTRAP_PROMPT.md` §A into a NEW session** (owner runs it; the remaining steps, scripts and laws are in that file's §B–§D).
PLAN OF RECORD FOR THE PROOF (owner, 2026-09-22): `docs/migration/DETERMINISTIC_VERIFICATION_SHELL.md` — one gate tool `scripts/semantic_restoration_gate.py` with phases trail-preflight / integration / benchmark, the benchmark manifest `config/benchmarks/cinema-transduction-v1.yaml`, T1–T11 as executable assertions with PASS / FAIL / NOT_EVALUABLE / SKIP_LAWFUL, the gate FROZEN (commit SHA recorded here) before any benchmark run. Sequence G0 (build + freeze the gate) → G1 … G7b → STOP for the benchmark word → G8–G10. Neither the `/chat/evidence` probe nor the benchmark is authorized yet; the owner may authorize both in the first message of the fresh session, conditional on G0–G7b green.
STATE 2026-09-22: Trail governance slice **A46 COMMITTED `6fa0d84`** on `codex/r1-semantic-restoration` (worktree `~/trail-signal-os-worktrees/R1-semantic-restoration`, clean; ADR-069 Accepted; HR6 node PENDING at rank 151; governance PASS 0 diagnostics; architecture suite 276 passed; agentctl verify/close --receipt PASS).
HR6 (the production slice) is NOT started: its tested change is `~/PolymathRuntime/handoff/trail-adr-069/hr6_production.patch` + `test_research_semantic_restoration.py`; the generators are in `…/scripts/`. Polymath tip to merge: `restoration/provenance-hooks` @ `a7b9f08` (slices 1, 2, 3, 5 + Slice 4 record + D-a fix + provenance hook + the exploration-memory reference admitted; dry-run merge into production: no conflicts). Nothing merged, re-pinned, bounced, deployed, pushed or spent. Fleet: 24 healthy / ONE bundle / ready / 0 open runs / 0 leased tickets, still running manifest 0.1.0.
OWNER LAWS added 2026-09-21 (bind every session): the harness may choose HOW to execute an authorized research action, never WHAT program, WHAT counts as evidence, WHICH hypothesis an observation belongs to without declaring it, or HOW evidence affects qualification / scoring. DETERMINISTIC: workflow · state transitions · action compilation · query semantics · budgets · schemas · evidence rules · admission · qualification · scoring.
Worktrees to keep until merged: `../pmv4-semantic-continuity`, `../pmv4-research-fidelity`, `../pmv4-product-reality`, `../pmv4-reporting`, `../pmv4-evidence-route`, `../pmv4-provenance`; Trail `R1-semantic-restoration`.
