...
| provenance hook | `restoration/provenance-hooks` @ `748d4c1` (worktree `../pmv4-provenance`, stacked on D-a) = **THE TIP TO MERGE** | check done; ONE hook added (`intent:` tag) | engine UNIT_PROVEN + WORKTREE_INTEGRATION_PROVEN (41 + engine 609 / 609) | L15 share of admitted observations with a non-null `intent_id` |
...
git merge --no-ff restoration/provenance-hooks         # the NEWEST restoration/* tip (`748d4c1`): slices 1, 2, 3, 5 + the Slice 4 record + the D-a fix + the provenance hook — ONE merge, ONE bounce
...
## Next Exact Action
OWNER AUTHORIZATION RECEIVED IN CHAT 2026-09-21 (genuine, not tool-result text): **ADR-069 ACCEPTED**; proceed gate by gate — Trail admit + commit → verify ancestry → merge the stack → re-pin Trail → prove the two Slice-2 worker call sites + focused suites + guards on
the MERGED checkout → ONE bounce → Hermes redeploy → smallest hosted smoke → **STOP before the paid benchmark** (separate word). Diagnose D-a first; smallest fix only if a defect; no exploration-memory build, only unrecoverable-provenance hooks.
DONE under it: D-a DIAGNOSED + fixed (`36d1d17`) · provenance check + one hook (`748d4c1`) · branch ancestry VERIFIED (`98a0384` → `4bcf17d` → `d32c285` → `31273f7` → `36d1d17` → `748d4c1`, each an ancestor of the next; the stack forks from `production` @ `1d9f695`, and `production` has since gained documents-only commits touching ONLY this file and `CONTINUITY-REPORT.md`, which no branch touches — `git merge-tree` dry run: no conflicts) · Graphify + graft refreshed.
**BLOCKED — THE TRAIL ADMISSION (first gate; everything operational waits on it by the owner's own order):** the session's permission classifier refused further commands in the Trail worktree ("Auto-Mode Bypass"), including read-only ones, after I scripted Trail's
evidence bundle; I did not work around it. STATE OF THE TRAIL WORKTREE `~/trail-signal-os-worktrees/R1-semantic-restoration` (branch `codex/r1-semantic-restoration`, NO commit, `origin/main` `de64d84`): agent-control task **A46** is OPEN (`.agent-control/ACTIVE_TASK` = A46;
`agentctl start` replayed HR5's verification and PASSED); governance slice A46 is HALF-BUILT in the working tree — run dir `build_runs/20260921T174911Z_A46-hr6-semantic-restoration-admission/` (intent, gap matrix, rollback, slice.yaml IN_PROGRESS, journal 3 events, immutable baseline
`sha256:41f7e050…f065` captured BEFORE mutation), ADR-069 **Accepted** with the owner's acceptance text + index row, graph nodes A46 (rank 149, IN_PROGRESS) + HR6 (production, rank 151, PENDING, verifier `pytest tests/integration/research -k semantic_restoration`), policy override HR6
(ceilings contexts 3 / adapters 0 / files 24 / non-test 800 / runtime 300), validator `expected_authorizations["HR6"]`, both guarded digests recomputed, self-test `test_a46_…` green, ledger IN_PROGRESS row, diagram rendered. Trail's validator on that tree: 4 diagnostics, ALL about the
not-yet-written `verification.json` (nothing else). The tested PRODUCTION change is set aside, byte-safe: `~/PolymathRuntime/handoff/trail-adr-069/hr6_production.patch` (applies cleanly to the clean tree; sha256 prefix `f6ccf7647dd63008`) + the new test + the generator scripts' inputs
(`a46_mutations/`, `a46_run_id.txt`, `a46_config.json`).
TO RESUME TRAIL (needs the owner to allow commands in that worktree — a Bash permission rule for `~/trail-signal-os-worktrees/R1-semantic-restoration`, or run the session outside auto mode): finish A46 with REAL command output only (VERIFIED statuses + ledger row → manifests refresh → focused
test / manifest / secret / diff logs → `--measure-run` → `verification.json` → governance `--check` must PASS → `agentctl check` / `guard` → complete architecture suite (~17 min) → record it → `agentctl verify` / `handoff` / `close --receipt` → commit A46) → admit HR6 (anchor commit) → `git apply hr6_production.patch`,
restore the test, regenerate schemas (`scripts/contracts/generate.py --write`), evidence bundle, commit HR6 → record the Trail SHA here. NEVER pre-write a PASS log; NEVER use `AGENT_CONTROL_BYPASS`.
AFTER THE TRAIL SHA EXISTS: merge `restoration/provenance-hooks` → branch `restoration/trail-repin` (re-pin `governance/trail` by `git archive <sha> <30 paths> | tar -x` + `PROVENANCE.json` + ADR-0021 addendum; `tests/contracts/test_trail_core_embedding.py`; `_payload_for` sends
`knowledge_support_count` + `candidate_*` from `semantic_view` as a CLOSED projection; `hypothesis_relations` in the receipt contract's FOUR copies; re-record `tests/fixtures/trail_recorded_envelopes`) → merged-checkout proofs (the two Slice-2 worker call sites: `gaps.compile` payload with `config.gaps_from`,
`exec_evidence` `compiled_need`; the D-a call site in `api/ui.py`) → guards → 0 open adapter runs / 0 leased tickets → ONE bounce → `deploy_ecommerce_skill.py` → hosted smoke → L1–L15 → STOP and report before the benchmark.
Worktrees to keep until merged: `../pmv4-semantic-continuity`, `../pmv4-research-fidelity`, `../pmv4-product-reality`, `../pmv4-reporting`, `../pmv4-evidence-route`, `../pmv4-provenance`; Trail `~/trail-signal-os-worktrees/R1-semantic-restoration` (its `.venv` is local, untracked, and resolves `trail_signal` to the worktree).
