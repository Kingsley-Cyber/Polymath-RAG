# POLYMATH CONSOLIDATION — BOOTSTRAP CONTEXT (EXTENDED, earlier owner text)

> Owner-authored, received in chat on 2026-09-20 BEFORE the bundle `polymath_migration_bootstrap.zip`. The bundle's compact
> `BOOTSTRAP_CONTEXT.md` is the CONTROLLING document; where the two differ, the bundle wins (decision M-003). This file is kept
> because it states details the compact set does not restate: the starting repository states (§3), the locked target tree
> `adapters/ecommerce/` + `governance/trail/` (§6), the graph-preservation principle (§8), the Trail import list (§9), the
> privacy exclusions (§10) and the report acceptance list (§16). Substance unchanged; only markdown structure was restored.
