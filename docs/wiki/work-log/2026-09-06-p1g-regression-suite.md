...
| 10-carry-contamination carry contamination (off-topic turn-1 evidence never reaches turn 3) — carry gate_no_leak | false (p0e-v1-baseline) | true (p0e-v2) | == true | PASS |
| 10-carry-contamination carry contamination (off-topic turn-1 evidence never reaches turn 3) — turn-1 chunks in turn-3 legend | 0 (p0e-v1-baseline) | 0 (p0e-v2) | ≤ 0 | PASS |
| 10-carry-contamination carry contamination (off-topic turn-1 evidence never reaches turn 3) — turn-1 chunks in turn-3 prompt | 5 (p0e-v1-baseline) | 0 (p0e-v2) | ≤ 0 | PASS |
| 10-carry-contamination carry contamination (off-topic turn-1 evidence never reaches turn 3) — carried items admitted per turn (max over turns) | — | 0 (p0e-v2) | ≤ 8 | PASS |
| 10-carry-contamination carry contamination (off-topic turn-1 evidence never reaches turn 3) — turn-3 prompt chars ≤ v1 baseline | 47321 (p0e-v1-baseline) | 38731 (p0e-v2) | ≤ 47321 | PASS |
...
5. **Turn-3 prompt size.** The LLM-synthesizer carry probe (chat-carry-final-llm) keeps gate_no_leak true with 0 turn-1 leaks and 3 carried items, but its turn-3 prompt (50,578 chars) exceeds the v1 baseline floor (≤ 47,321) that case 10 freezes — the P1.a–P1.c evidence set (15 rows + coverage lines + request block) is larger than the P0.e bundle. Refresh refused; the owner decides whether the floor moves to the P1 evidence contract or the prompt is trimmed.
