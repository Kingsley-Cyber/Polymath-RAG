...
- Live acceptance attempts 5-10 (after the gate fixtures and single-hypothesis / cause θ fixes): the loop reached `opportunity.score` and Trail refused with `HARD_GATE_UNMET` because the qualify sent `admitted_evidence_ids: []`. Root cause in the service context builder, not the wire: `_context_refs` filled all 200 context slots with the cinema corpus's knowledge refs (111 chunks + 54 graph facts + …) before any Trail-admitted field evidence, so the admitted evidence was truncated out and the qualify/score derived an empty admitted set (Trail then correctly returned NO_DEFENSIBLE_BRIDGE). The determinism stub loop never hit it (2 knowledge refs). Fixed: `_context_refs` adds Trail-admitted field evidence first. Alongside: the scripted θ develops one hypothesis and cites a guaranteed-allowed cause, and the fixtures satisfy every gate.
