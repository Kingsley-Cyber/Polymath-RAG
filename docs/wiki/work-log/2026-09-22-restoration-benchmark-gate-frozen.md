...
## Proof
Gate 1.0.0 was committed at `2167569` and superseded the same hour, before any benchmark run existed, by 1.0.1 (the markdown verdict under `docs/wiki/` carries front matter — `wiki_worm` was red on `2167569`); no assertion changed. EXECUTED on the merged main checkout, DB-free (`env -u POLYMATH_PG_DSN`): `tests/determinism/test_semantic_restoration_gate.py` 25 / 25; the run-5 export itself was one read-only SELECT session against the fleet's database (no write). Guards 0 / 0 / 0 / READY. Proof level: UNIT_PROVEN (the gate is pure; its executed path IS this checkout). The gate's `integration` phase was proven at G4 on this checkout (13 / 13 PASS); `trail-preflight` on the committed HR6 bundle (10 / 10 PASS).
