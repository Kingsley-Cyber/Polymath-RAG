...
1. **Attribution of the full suite**: worktree run showed `6 failed, 2996 passed`; 3 of the 6 (`test_fact_endpoint_eligibility::…pronoun_endpoint`, `test_killchain_pass2::…identifiers…`, `test_query_receipts::…wired`) + `test_chat_runtime::test_compiler_on_drives…` + `test_chat_hygiene::…skips_retrieval…` fail IDENTICALLY on untouched `production` (pre-existing). Two background runs were writing the FULL `-rf` lists to `/tmp/gnn_suite_worktree.txt` and `/tmp/gnn_suite_production.txt` (`/tmp/gnn_suites_done` appears when both finish; ~13 min each). Compare the two `FAILED` lists — the worktree list must be a subset of production's. If a failure is worktree-only, it is ours: fix before merging.
