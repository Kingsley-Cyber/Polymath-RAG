...
| 6 | groq/compound is an agentic system: each request is 2–3 internal model calls (`usage_breakdown`: llama-4-scout router steps + gpt-oss-120b answer), so per-key RPM is consumed 2–3× and the dashboard attributes usage to those models. A plain instruct model (gpt-oss-120b directly: 3.5 s, same format) would halve rate pressure | owner's model choice | **Yes** — the owner named compound; keep unless told otherwise |
