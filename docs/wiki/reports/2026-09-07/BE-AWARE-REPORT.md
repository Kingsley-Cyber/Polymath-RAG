...
| Isolated pool: six dedicated Groq accounts (groq/compound) tier 0 → Gemini fallback 1 → OpenRouter fallback 2 | OWNER | "6 dedicated api keys for this lane, keep gemini as fallback 1". **Why compound and not a plain instruct model:** on the Groq free plan `groq/compound` has RPM 30 / RPD 250 / TPM 70K and NO daily token cap, while `openai/gpt-oss-120b` and the Qwen models have TPM 8K / TPD 200K — about one profile request a minute per key. Measured 2026-09-07 with the rate-limit headers: one profile request = 12 internal model calls (11 llama-4-scout router steps + 1 gpt-oss-120b answer), 38.5k internal tokens, 40 s, but the key's own TPM window is charged ~3.7k and RPD 1. The dashboard shows the internal models; the 429s come from their budgets → limiter rows rpm 2 / conc 1 per slot (per key). Do not "optimise" the lane onto gpt-oss: it is rate-limited out. |
