...
| 5 | **L4** canary + live proof — **L4a canary DONE 11.468** (23 / 23 lanes); L4b (one document through the fleet) waits on the owner's choice of document | L-03 | L3 | ≤ 20 Groq + ≤ 6 Cloudflare calls (OTPM per org, TPM reservation, each pair reachable); then one profile ticket + one small pMAP document land only on their owning pairs | the owner's word before the first model call (the L3 prompt, 2026-09-24: "Stop before the L4 test calls and ask me") |
