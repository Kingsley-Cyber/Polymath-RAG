...
| 6c | **K1b** the scope echo + Trail's check | K-04 | K1 | every scoped response echoes the applied scope (`meta.knowledge_scope`); Trail refuses a response without the exact echo, so a rollback to pre-K1 code fails closed instead of widening; lands before C1's importer writes its first implementation document | fence + bounce |
