...
| 3 | **L2** limiter correctness — **DONE 11.466** | L-04, L-05, L-08, L-09, L-16, L-17 | L1 | unit tests: real-token admission, rolling TPD, OTPM, clamp on restore, transient refusal; ledger rows carry stage / account / model | fence + bounce |
| 4 | **L3** ownership + wiring — **DONE 11.467** (L-11 waits on the owner's Cloudflare account ids) | L-02, L-07, L-11, L-14, L-15, L-20 (L-12, L-13, L-18 closed early by 11.464) | L2 | registry: all 18 Groq pairs owned (6 profile + 6 pMAP slots), Cloudflare account ids discovered (read-only API) or supplied by the owner, dead lanes parked, the unused flag removed | `.env` edits (account ids) = owner-visible; fence + bounce |
| 5 | **L4** canary + live proof — **DONE 11.468 + 11.474** (L4a: 23 / 23 lanes; L4b: a one-document test corpus, one call per model stage, each 200 on its owning lanes, pMAP 1 / 1 mapped) | L-03, L-19 | L3 | ≤ 20 Groq + ≤ 6 Cloudflare calls (OTPM per org, TPM reservation, each pair reachable); then one profile ticket + one small pMAP document land only on their owning pairs | the owner's word before the first model call (the L3 prompt, 2026-09-24: "Stop before the L4 test calls and ask me") |
| 6 | **D1** document defects — **DONE 11.478** | D-01, D-02 (marker) | — | GRAPH hop-1 facts ranked by seed rank × predicate tier × evidence in the selected set (replay on saved GRAPH plans shows the change); MCP rows say `truncated` + full length | flag; fence + bounce |
| 6b | **K1** knowledge roles + retrieval scope — **DONE 11.485** (retrieval side; the column + importer roles move to C1, K-03) | K-01, K-02 | — | a reference-only request never sees implementation material in any lane, the profile scout or the compiler context; Trail always sends `roles: [reference]`; scope in cache identity (CODE-RAG-IMPLEMENTATION-V1 §4 K1) | Qdrant backfill in a live window |
| 6c | **K1b** the scope echo + Trail's check — **DONE 11.487** | K-04 | K1 | every scoped response echoes the applied scope (top-level `knowledge_scope`); Trail refuses a response without the exact echo, so a rollback to pre-K1 code fails closed instead of widening; lands before C1's importer writes its first implementation document | fence + bounce |
| 6d | **AUTORESEARCH-SOURCES-AND-HARNESS-V1** (the owner's order 2026-09-25, before C1): TikTok / short-video comments admitted and counted, CJ alongside Alibaba, any MCP harness can run the governed product research, one real e2e run — slices R0–R7 in `AUTORESEARCH-SOURCES-AND-HARNESS-V1.md` | S-01..S-07, H-01..H-07, A-03..A-07 | K1b | the plan's §4 acceptance per slice; R7 = one real run with TikTok comment observations ADMITTED and CJ + Alibaba listings admitted in supply | Trail re-pin + fence + bounce |
