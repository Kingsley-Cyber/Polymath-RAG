...
| [tree-sitter-grammars/tree-sitter-luau](https://github.com/tree-sitter-grammars/tree-sitter-luau) | MIT | v1.2.0 (last push 2026-09-13; 7★) | **Fallback / comparison only** (owner 2026-09-24): the official `luau-ast` is the Luau parser. Kept for the C0 comparison and as a fallback if `luau-ast` cannot run | pip wheel | C0 (compare) |
...
| [luau-lang/luau](https://github.com/luau-lang/luau) | MIT | 0.739 (2026-09-18) | **The official Luau toolchain, the Luau parser of record** (owner 2026-09-24): `luau-ast` (build target `Luau.Ast.CLI`, CMakeLists L66–L74) prints the official, fully typed AST; `luau-analyze` type-checks and lints. C0 checks whether `luau-macos.zip` ships `luau-ast`, else it is built from source | checksummed binary release or a pinned source build | C0, C4, C13 |
...
|---|---|---|---|---|---|
| [rojo-rbx/rojo](https://github.com/rojo-rbx/rojo) | MPL-2.0 | v7.7.0 | **Required for Luau** (owner 2026-09-24). The standard file ↔ instance mapping for file-based Roblox projects (community-made, not by Roblox itself). `rojo sourcemap` builds the instance tree that resolves `require(script.Parent.X)`, and its naming convention gives script kinds (server / client / module) | unmodified binary | C0, C4 |
