...
| P3 | `D_discover` = `discover.submit` via `_exec_discover` (`adapter_step_worker.py:266`) | manifest v1 + ADR-0018 §5 | HARNESS_ACTION AGENT_RESEARCH issued from `gaps.compile` | ADR-0019 §1/§2 | no | yes (manifest) | no | R4: live run issues an AGENT_RESEARCH action and resumes on receipt | `_exec_discover`, `_hypothesis_queries`, `discovery_request` (client builder) removed at R4 | symbol sweep §12 returns 0 hits in runtime+tests+docs (except history) | REMOVED (R4, 11.268) |
| P4 | `E_acquire` = `scrape.submit`+`extract.submit` via `_exec_acquire_extract` (`:289`) | manifest v1 | harness receipt → `evidence.admit` | ADR-0019 | no | yes | no | same as P3 | `_exec_acquire_extract`, `_leads_from_prior`, `batch_request`, `extraction_request`, `crawl_request` (already dead), gap codes TRAIL_NO_LEADS/TRAIL_DISCOVERY_*/TRAIL_ACQUISITION_* removed at R4 | sweep §12 | REMOVED (R4, 11.268) |
| P5 | `F_normalize` (agent maps Trail records into the ontology) | manifest v1 | removed; Trail `registry.project` + `evidence.admit` do it deterministically | ADR-0019 §1 | no | yes | no | R4 manifest | step id gone; `trail_record` evidence kind removed from `adapter_step.schema.json` enum when no manifest uses it | sweep `trail_record_ids` = 0 | REMOVED (R4, 11.268) |
| P6 | `G_gates`/`I_score` planned `commerce.research.*` (Trail C1/C2) | ADR-0018 §5, Trail graph C1/C2 | `evidence.admit` + `opportunity.qualify` + `opportunity.score` (Trail HR2/HR3) | ADR-0019 §8, ADR-063 | no | yes | no | Trail HR3 WORKING + live run reaches score | `planned_node: C1/C2` references gone | manifest has no `availability: planned` steps after HR3 | MANIFEST CUT_OVER (R4); Trail HR3 pending |
| P7 | `H_gap_loop` → `D_discover` (loop into Trail acquisition) | manifest v1 | BRANCH → `revise_hypotheses` → `gaps.compile` (loop through Polymath reasoning) | ADR-0019 | no | yes | no | R4 manifest test: no branch targets a HARNESS_ACTION directly | — | pure-core test G | CUT_OVER (R4, 11.268) |
| P8 | `exec_external` dispatch on operation kinds (`:236`) | E4 | `exec_external` dispatches by `external.operation_kind` to the bounded ops; two-phase pending path retained for any Temporal op | ADR-0019 §8 | no | no | no | R4 | Trail acquisition kinds removed from the dispatch | sweep | CUT_OVER (R4, 11.268) |
| P9 | `trail_client.py` builders `discovery_request/crawl_request/batch_request/extraction_request/page_request/operation_reference/cancel_command` | E4 | keep `TrailMCPClient` + JWT + envelopes; builders for the 7 bounded ops; acquisition builders removed | ADR-0019 | no | yes | no | R4 | acquisition builders + their tests (`test_trail_client.py:30-63`) replaced | sweep | REMOVED (R4, 11.268) |
...
| P11 | `_TOOL_NAMES` research_* in `mcp_server.py:586-591`; prompt text `:73-74` | E6 | 7 `adapter_*` tools only | ADR-0019 | no | yes (Hermes prompt) | no | R5 | with P10 | `test_mcp_server_v2` tool list | DONE (R5: register 11.273-11.274) |
...
| P15 | `tests/determinism/test_adapter_trail_connector.py` (stub Trail discover/scrape/extract) | E4 | replaced by stub tests of the 7 bounded ops + receipt admission | ADR-0019 | no | yes | no | R4 | old test removed with P3/P4 | CI | REMOVED (R4, 11.268) |
| P16 | `config/adapters/trail.product_discovery.json` 1.0.0 (A–K) | manifest v1 | 2.0.0 on the §2 loop (same file, same adapter_id; workflow_version 2.0.0; output_schema §3.4) | ADR-0019 | no live runs | yes | no (`adapter_list` shows one manifest) | R4 tests + live run to the first Trail gap | v1 step ids gone | manifest test | CUT_OVER (R4, 11.268) |
| P17 | `contracts/adapter/v1/adapter_manifest.example.json` (= v1 trail manifest) | E1 | example = 2.0.0 manifest | ADR-0019 | no | no | no | R4 | — | contract test | CUT_OVER (R4, 11.268) |
...
| T4 | Trail graph C1/C2/Q1/C3 (PENDING, behind P9) | ADR-015 sequencing | SUPERSEDED; A32 + HR1–HR4 are the active commerce path; P9 not a prerequisite for the first loop | ADR-063 | no | no | no | A32 VERIFIED (commit 9e4e463, PR #4 under owner review) + A33 VERIFIED (commit 6040bf4, stacked PR #5: HR1–HR4 produces += lifecycle/manifest artifacts, ranks 127/128/129/131) | n/a (history retained) | governor DAG checks | PENDING (R3) |
...
| T8 | Trail stack (Temporal :7233, Postgres :15433, VersityGW :7070, daemon :8767) | environment | LOCAL stack up 2026-09-15 for R5: `handoff-drafts/trail_stack_up.sh` (compose project `trail-signal-local`, own volumes, local secrets in `~/.config/trail-signal/local.env`, v2 migrations 1000–1008 applied, daemon from the HR3 tip); the owner's stack/volumes untouched | — | — | — | — | O4 | — | — | LOCAL (O4 for the owner's stack) |
...
| R3 | Trail | ADR-063 + A32 governance; HR1 snapshot compiler + contracts + replay; HR2 admission/qualification/scoring + replay; HR3 bounded MCP ops + audit records; HR4 loop canary | R2 (contracts agreed) + owner D1/D3/D4 | 2026-09-15: HR1 (PR #8), A37 (PR #9), A38 (PR #10), HR2 fa7ddc0 (PR #11), A39 3f5f5c8 (PR #12), A40 520d942 (PR #13) committed under Trail governance and stacked for owner review; HR3 real chain from the A40 tip (this session); HR3 code proven live on the local stack (registry.project → 5 priors, byte-identical replay, audit row) |
| R4 | Polymath | manifest 2.0.0; executors for the 7 ops; remove P3/P4/P5/P8/P9/P15 surfaces; tests B, C, D(proj), E, H, I, J | R2 (+ R3 contracts for stubs) | DONE (11.267–11.270); 2026-09-15 R5 cutover flips the eleven EXTERNAL_OPERATION steps `planned` → `working` (11.272) |
| R5 | both | live acceptance through `scripts/adapter_mcp_acceptance.py` with a real harness executing actions; remove `research/`, `research_*`, workflow; final audit | R3 HR3 WORKING, O1, O4 | 2026-09-15 (11.272): manifest cut over; scripted receipts `tests/fixtures/harness_receipts/`; live acceptance against the local Trail daemon is the next proof; `research/` removal + Hermes symlink (O6) follow the acceptance |
...
|---|---|---|---|
| D1 | Which Trail engine is THE authoritative opportunity score | RESOLVED by the owner 2026-09-15 with the existing Trail scoring law: Trail alone owns the authoritative deterministic `opportunity.score`, composed only from admitted evidence after the hard gates by the retained doc-08 engine behind the v2 scoring context (HR2, byte-identical golden replay); model/harness confidence is never a score input; the rubric stays quarantined | HR2 (done) |
...
| O1 | `polymath` principal + `mcp.polymath.bearer` alias in Trail `config/v2/{principals,secrets}.yaml` (HR3); `TRAIL_SIGNAL_MCP_JWT_SECRET` or a minted token in the fleet env | LOCAL 2026-09-15: the fleet .env carries the local daemon's HS256 secret (never committed); the owner's production secret remains an owner action | HR3 live, R5 |
| O4 | Trail stack up on this host | LOCAL 2026-09-15 (see T8); the owner's own stack remains an owner action | HR3 VERIFIED, R5 |
