...
|---|---|---|---|
| D1 | Which Trail engine is THE authoritative opportunity score: the doc-08 five-axis engine (`signal_engine`, camping fixture 0.72) or the 13-dimension rubric (`scoring_weights.json`, bands) | Refined after the HR2 drafts: doc-08 normalises by percentile rank WITHIN A COHORT of candidates, so it cannot score a single run's opportunity on its own; the rubric scores one candidate from its dimensions. Recommendation: the run-level `opportunity.score` = rubric dimensions DERIVED deterministically from admitted evidence (counts, independence groups, gates, price/MOQ observations) — never typed by an agent; doc-08 stays the cross-run portfolio ranker over the findings corpus (a later slice). One score per authority level. | HR2 |
