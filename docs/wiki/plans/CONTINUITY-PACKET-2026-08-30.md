...
1b. Slice 2 (owner-approved design, compiler spec in session 3): one summary compiler replaces `retrieval_summaries.py` + `parent_summary.py` + `summarizer.py` parent path (background TF-IDF mandatory, coverage per child, Jaccard dedupe, source order, relations from facts, keywords, serialized embed text), per-card coverage receipts, `reprofile.v1` ticket (cards without re-extraction), S2 consumes the compiler, drop the unused `parent_summary` Qdrant cards. Decisions taken by the assistant with owner delegation: gate rule narrow (#6), S2 kept as a thin consumer (#10), old compiler tests superseded.
