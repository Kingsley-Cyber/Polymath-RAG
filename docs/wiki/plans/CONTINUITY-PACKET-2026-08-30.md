...
- **Session 3 rerun lessons (2026-08-30 09:19–10:10):** (1) LLM-DIRECT's `write_bundle` fail-closed on the sentence-slice manifest that `llm_live` never produces — every extract attempt failed and two full cloud passes were burned before the fix (`51dd9b2`). (2) The verify gate counters were concatenated onto id lists (`TypeError`) — fixed in the same commit. (3) A multi-file upload exposed the legacy-census/ticket race: SC-200's profile/canonicalize/neo4j/verify ran BEFORE extract (events without a ticket row passed the claim gate; census emitted every missing stage at once) — fixed by TICKET-GATE-FAIL-CLOSED-V1 + CENSUS-FIRST-GAP-V1. (4) Hand-started processes without the fleet env (`POLYMATH_QUERY_POLICY`, `POLYMATH_SYNTAX_PROVIDER`, …) compute a different execution contract: the orchestrator ran under policy v1 from 08:35 to 09:59, and a hand-started extract worker refused every lease. (5) A UI delete blocks on the extract stage transaction (12+ min); it now times out to 409 `runs_in_flight`. (6) `delete_corpus` left 904 projection receipts (routing-card/procedure/concept ids) behind while dropping the Qdrant collection — a re-ingest would have skipped re-embedding into a hole; purged, and the delete now removes every projected id.
