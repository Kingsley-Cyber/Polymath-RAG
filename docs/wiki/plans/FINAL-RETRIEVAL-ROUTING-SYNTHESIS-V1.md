...
| P5 | SEEALSO / BRIDGE fan-out resolver | **TERM BRANCH DONE (2026-09-08, 11.172)** — additive lane G (`SEEALSO_FANOUT`): the intent's RELATIONAL atom texts probe ORIGINAL children (global dense, coverage-free), LATENT role, default-off, flag-off byte-identical (`chat_regression` 131/0). Live: 24 fan-out children from 6 relational atoms, judged, 2 survive. Atoms route, children prove, judge decides. **SYSTEMATICALLY QUALIFIED LIVE 2026-09-08 (11.179):** fires on every cinema RELATIONSHIP query, 24 source-child candidates/query, coverage-INDEPENDENT confirmed (atom→global child). **STILL OPEN:** DOCUMENT branch (profile→MAP→children) reach → parent-MAP coverage; downstream answer-quality uplift → D-10. | 11.172, 11.179 · p5-seealso-fanout |
| P6 | Intent-conditioned GRAPH auto-routing | **DONE** (default-off, mode stays HYBRID, fail-open) | 11.164 · p6-graph-assist |
| P7 | Graph destinations through parent-MAP | **DONE (doc-level, 2026-09-08, 11.173)** — additive judged lane H (`graph_dest`): entity seeds → Neo4j hop (P6's `graph_expand`) → destination entities → `mentions`→docs → JUDGED children tagged `ARRIVAL_GRAPH_DEST` ⇒ RELATIONAL. Clears D-7 WITHOUT the risky reorder (pre-judge lane; `_attach_graph` untouched). Live: 8 destination docs, RELATIONAL role populated 0→1 in final evidence. Flag-off byte-identical. **SYSTEMATICALLY QUALIFIED LIVE 2026-09-08 (11.179):** fires on every cinema RELATIONSHIP query, 8 Neo4j-attested destination docs/query (2.4 s), coverage-INDEPENDENT. **STILL OPEN:** §39 parent-MAP localization (doc→parent) → coverage; P11 GRAPH uplift → density; 2.4 s latency tuning. | 11.173, 11.179 · p7-graph-destination |
| P8 | Synthesis evidence-role bundle (DIRECT/PRECISION/RELATIONAL/LATENT) | **DONE (role structure)** | 11.165 · p8-synthesis-roles |
| P8b | Synthesizer PRESENTS by role | **DONE (LLM path, 2026-09-08, 11.174)** — `_grounded_messages` groups the [S#] block by role (DIRECT→PRECISION→RELATIONAL→LATENT), labels each tag, prepends role guidance (§47) behind `POLYMATH_CHAT_SYNTH_ROLES`. Done in the LLM prompt path, so `answer_synthesis.py` (the claim system D-8b feared) is UNTOUCHED. Default-off byte-identical (`chat_regression` 131/0); citations unaffected. **OPERATIONALLY QUALIFIED LIVE 2026-09-08 (11.179):** on REAL multi-role retrieval evidence from a cinema bridge query (10 DIRECT + 5 LATENT), roles ON regroups DIRECT-first (measured: 0 DIRECT-after-LATENT), labels every [S#] by role, and prepends the §47 guidance ("never let LATENT/RELATIONAL substitute for a DIRECT answer"); roles OFF byte-identical; the [S#]→locator mapping is unchanged (citations stable). Roles are NOT thin on relational queries (5 LATENT here). **STILL OPEN:** deterministic `grounded_answer` by role (the /ask path); FORMAL answer-quality uplift number (LLM-judge, shared D-10 gap). | 11.174, 11.179 · p8b-synthesis-roles |
| P9 | Task-conditioned breadth (no quotas) | **DONE** (default-off) | 11.166 · p9-breadth |
| P10 | Measure HYBRID | **NON-REGRESSION PASS + LANES QUALIFIED LIVE (2026-09-08, 11.179).** Intent stack ON vs OFF on cinema: L exact 15/15→15/15, B grounded 13/15→13/15, **0 regressions** (§53). Beyond non-regression, the intent lanes systematically EXPAND the candidate union — L avg 69.5→109.5 (+58%), B 90.9→147.8 (+63%); on RELATIONSHIP queries all four additive lanes fire + contribute SOURCE-CHILD candidates: **P5 seealso_fanout 24** (via 6 relational atoms, e.g. "linking cinematography to film editing"), **P7 graph_dest 8** (Neo4j-attested destination docs, 2.4 s), **F resolution_lift 6** (lifted terms B1/C3/C500), **E dual-read 22-24** (cinema's 838 parent-maps) — union +60-75%, gold preserved, candidates are `routing_child` (atoms/graph ROUTE, children PROVE; §63/§64 invariant held). P5/P7/F confirmed coverage-INDEPENDENT (atom/graph → global child). Evidence: `docs/wiki/experiments/routing-lanes-qualify-cinema-2026-09-08.json`. **JUDGE-SURVIVAL finding (honest limit):** the relational candidates expand the UNION but do NOT survive the cross-encoder into final evidence on the L/B fixtures — final top-15 is all `DIRECT` (`evidence_roles {DIRECT:15, RELATIONAL:0}`), even on RELATIONSHIP queries. The judge correctly prefers direct dense hits when cinema HAS them. **VALUE DEMONSTRATED (2026-09-08):** on cross-document / exploratory queries where direct hits are SPARSE, the relational/latent candidates DO survive the cross-encoder into final evidence — editing↔fermentation **5 LATENT**, hydrology↔composition **3 LATENT**, maintenance↔cinematography **2 RELATIONAL** (graph_dest) survivors per query. So the lanes provide real evidence exactly where direct is insufficient AND correctly yield to direct otherwise — the designed behaviour, invariant-preserved (children prove, judge decides). **D-10 ANSWER-QUALITY MEASURED (2026-09-08, 11.181):** the answer-side uplift, over the free Ollama `gemma4:31b-cloud` on 5 relational cinema queries (lanes ON vs default OFF, GENERATED answers): a blind A/B LLM-judge (randomized order) prefers the lanes-ON answer **4/5** ("better explains the connection"), and the ON answer objectively CITES relational-role evidence (LATENT/RELATIONAL/PRECISION `[S#]`) in **3/5** queries — evidence the DIRECT-only default retrieval could not supply (e.g. colour→film-music "coloristic effects", S5). Caveat: small sample + LLM-judge (not a human rubric), but a measurable answer-side uplift consistent with the objective cross-encoder-survival result. Evidence `docs/wiki/experiments/d10-answer-quality-cinema-2026-09-08.json`. **Residual:** scale the relational fixture + a human/larger-model rubric for a statistically-tight number. | 11.167, 11.179, 11.181 · production-routing-qualify |
| P11 | Measure GRAPH | **QUALIFIED LIVE (2026-09-08, 11.179).** GRAPH mode on cinema attaches **15 source-attested Neo4j facts** (predicates e.g. `PRODUCES`, subject/object entities) via 8 entity-card seeds (card_probe ok), bounded hop-1, clean (`graph_degraded` null). Facts ride the `graph_relationships` side channel — relationships ROUTE, children PROVE (invariant held; facts never enter evidence). Graph density suffices for cinema. **STILL OPEN:** downstream answer-quality uplift from the facts → D-11 (relational fixture). | 11.173, 11.179 · p7-graph-destination |
| P12 | Wildcard over profile atoms | **IMPLEMENTABLE (data gate cleared 2026-09-08, 11.169)** — atoms exist; NOT wired. **Exact next:** add an atom-frontier source to `divergent_sweep` (`shared/polymath_shared/divergent.py`) alongside latent_abstraction/latent_transfer — `search_atoms` for the wildcard kinds (LATENT_PATTERN/ANCHOR/BRIDGE/TENSION/INVERSION) → atom-nominated parents → merge into the sweep slots → `divergent_finish` (novelty/support unchanged). Additive, WILDCARD-only, default-off. **Value coverage-gated** (bridges need child coverage) + narrow reach (WILDCARD is opt-in) → deferred as the lowest-value/highest-complexity routing slice. | was dep: vNext atom regen (D-12, satisfied); now dep: divergent-sweep atom-frontier slice + coverage |
| P13 | Measure Wildcard | **WILDCARD MODE (latent frontier) QUALIFIED LIVE (2026-09-08, 11.179).** `chat_retrieve_mode("WILDCARD")` on cinema: the divergent sweep fires (45 / 42 latent_candidates) → obvious-excluded → two-hop validated (parents_validated 1–5) → **3 bridges returned** on the separate `wildcard` lane (never evidence, §3.19). Coverage-INDEPENDENT (latent frontier). **STILL OPEN:** P12 atom-frontier ADDITION (lowest-value, unimplemented); downstream novelty-quality uplift → fixture. | 11.179 |
