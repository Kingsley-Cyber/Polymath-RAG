...
## Latest checkpoint (2026-09-18 — AGENT-SAFETY STACK installed + Librarian audited CLEAN; ELITE settled, librarian rebased/renumbered, P5 contract frozen + P5a done; NEXT = P5b scout wiring)

**Branch:** `librarian/retrieval-architecture` (worktree `pmv4-librarian`), tip **8b7b750**, off the ELITE base `4d95da2`. UNMERGED. Register runs to **11.292**. Production is `557389d` and its CONTINUITY names the older librarian tip `8f044b2` — THIS branch's checkpoint is authoritative for the active work. Read `docs/wiki/plans/PROFILE-SCOUT-V1.md` (the P5 contract) + `architecture/contract-dependencies.yaml` (the contract map), then this block.
...
**NEXT ACTION — P5b (scout wiring + title retirement), in `orchestrator/orchestrator/api/ui.py`:** run `projection.profile_nominate` + `profile_atom_projection.search_atoms` for q0's vector, NORMALIZE into `ScoutHit`s (profile → `{doc_id,rank}`; atom → rich + SurfaceRegistry group), call `fuse_profile_scout_hits` BEFORE `compile_plan`, thread `PrePlanContext {q0, profile_scout_result}`, and RETIRE `_compiler_titles` (`ui.py:1671`/`:1764`) atomic with the wiring. Flag `POLYMATH_PROFILE_SCOUT` default-off-in-code / on-in-`.env`, fail-open, bounce-gated; observable receipt. Then `P6 subquery provenance → P10 → P11 → integration → live E2E (L1–L5)`. Do NOT: apply 0065, bounce the fleet, or start graph traversal / multi-hop.
