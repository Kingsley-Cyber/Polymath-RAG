...
- **current implementation slice** — the deterministic map-production CORE is built and tested (all pure `shared/`, no API): **S0 admission** (11.129), **S1 ParentSkeleton** (11.130, 13 pins), **S2 parent-map compiler** (11.131, 15 pins), **S3 token packer / capacity model** (11.132, 12 pins: `map_batches.py` — density EMA seeded 135.7/87/29, ~60 cap, combined formula, TPM guard, deterministic batch manifests). **S4 (SQL durability) is the next executable slice.** Slice ledger: S0…S16 in the plan §40.
