...
**REMAINING to full DONE_AND_PROVEN** — (1) **P10 resolution live-wiring** — the shared core is UNIT_PROVEN (`evidence_resolution.py`, 13 tests: bounded no-runaway, q0 preserved, stop conditions) but NOT wired into ui.py's live evidence bundle (a bounded 2nd `chat_retrieve_mode` round whose new evidence feeds synthesis). Gap-detection needs the round-1 evidence texts (the legend carries doc_ids/breadcrumbs, not text) OR the existing `weak_aspects` signal — a design choice needing live iteration, deliberately NOT blind-wired. Demonstration-need: `res_shutter_motion`. (2) restore the stashed frontend/ELITE stream (`git stash pop` — dry `apply --check --3way` is CLEAN against the merged tree; if it ever conflicts, keep the stash + use the recovery snapshot, never force). Everything else (P5/P6/P11 + modes + migration + bounce + health) is LIVE-PROVEN. Flags live: `POLYMATH_PROFILE_SCOUT=1`, `POLYMATH_CHAT_PROFILE_EXPANSION=1`.
