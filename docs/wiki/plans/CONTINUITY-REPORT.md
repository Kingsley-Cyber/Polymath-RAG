...
## CURRENT — 2026-09-19 — **WLK2C-RETRIEVAL-LINEAGE-V1 = V1 baseline, COMPLETE + causally PROVEN** (merged, flags OFF, NOT qualified as final); **LATENT-QUERY-FUSION-V2 ADMITTED (owner /goal) — query-stratified multi-stage fusion is the real spine**; CA0–CA5 COMPLETE; WLK2A/2B done

**Repository State** — branch `production`, HEAD **d904154** (WLK2C merged 86196eb + C7 pool-fix), tree CLEAN. WLK2C worktree `pmv4-wlk2c` (branch `wlk2c/retrieval-lineage`) MERGED. Stash `49f57e0c` (frontend/ELITE) UNTOUCHED. **Fleet: bounced onto d904154, WLK2C flags OFF (`POLYMATH_CHAT_BRIDGE_COMPILER=0` + `POLYMATH_CHAT_LATENT_SELECTION=0` in .env — verified in the live process env), 11 healthy/one bundle, `/ready` true → proven pre-WLK2C no-op behavior.** LIVE flags: CA3/CA4 + scout/expansion/resolution/intent-policy (all pre-WLK2C, on). WLK2C V1 is the flagged BASELINE for A/B against V2.
...
**Next Action** — **execute LATENT-QUERY-FUSION-V2** (plan-of-record `docs/wiki/plans/LATENT-QUERY-FUSION-V2.md`). Order: **F1 (representation/observability ONLY)** — a `RankedLane` per query capturing each query's LOCAL ranked list + preserve multiple lane memberships per chunk; a receipt showing e.g. "bridge A ranks Laban #1"; flag-off behavior byte-identical (NO selection change) → then **F2** deterministic lineage-aware weighted RRF (bounded local-winner preservation through `merged_candidate_max`/`judged_prefix`) → **F3** feed the fused set to the existing C4→C5→CA4→synthesis → **F4** merge+bounce+A/B (V2 vs V1 vs pre-WLK2C) on WLK-10 + survival + main harness + CA5 64×4. Flag `POLYMATH_CHAT_LATENT_FUSION=0`. Investigate first (candidate_engine union fusion l.968–1052, `judged_prefix` l.1265–1321, `aspect_prefix_seats` l.1339–1347, `query_scores`/`dense_rank`/`sparse_rank`). wc01/wc05/wc07 = DIAGNOSTIC PROBES, not optimization targets. **Do NOT re-run/reopen WLK2C/C7.** SEPARATE owner decisions: WLK1/WLK3; restore stash `49f57e0c`.
