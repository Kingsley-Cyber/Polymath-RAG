...
Read order: this file → **`docs/wiki/reports/2026-09-09T2039/START-HERE.md` (the NEWEST dated handoff snapshot — end-of-session 2026-09-09; supersedes `2026-09-09/` and `2026-09-08/`) + its `BE_AWARE.md` / `UNFINISHED_WORK.md` / `DEPENDENCY_MAP.md`** → **`docs/wiki/plans/FINAL-RETRIEVAL-ROUTING-SYNTHESIS-V1.md`** (the LIVING plan-of-record ledger: phase table P0–P14 + primitive table R1–R10 + DEFERRED register D-5…D-14) → `docs/wiki/plans/RETRIEVAL-MIGRATION-DEPENDENCY-V1.md` (migration/retirement authority) → `docs/wiki/plans/PLAN-AUTHORITY-REGISTER.md` (append-only; `production` runs to **11.280** (ELITE), the `librarian/retrieval-architecture` branch adds **11.281–11.286** (unmerged); read the newest rows) → `POLYMATH_EXECUTION_AUTHORITY_XML_FINALIZED.md` (`~/Downloads/`, the current owner execution authority — supersedes the prior directive on anything it names explicitly) → `CLAUDE.md` → the two newest work-logs → **`Polymath_librarian_architecture_checklist.md`** (repo ROOT — the ACTIVE Librarian-goal contract + ground-truth ledger; see the 2026-09-18 checkpoint below) → **`git worktree list`** (in-flight work lives on `librarian/retrieval-architecture`). See the **NEXT SESSION** block at the end of this checkpoint for the exact read order + first action.

## Latest checkpoint (2026-09-18 — ELITE-MODE SETTLED as the authoritative retrieval base `4d95da2`; LIBRARIAN foundation REBASED onto it, register collision CLEARED (renumbered 11.281–11.286); six slices green on the new base, UNMERGED; retrieval-flow core P5→P6→P10→P11 is the remaining work, gated on inspecting the post-ELITE retrieval code)

**Read first:** `Polymath_librarian_architecture_checklist.md` (repo ROOT — contract + ground-truth ledger), then this block. Owner directive 2026-09-18: **"Absorb ELITE first"** — settle the concurrent ELITE-MODE stream into ONE authoritative retrieval topology, then build the remaining Librarian architecture on top of it.

**ELITE-MODE SETTLED → `4d95da2` on `production` (the new authoritative retrieval base).** The previously-uncommitted ELITE-MODE-RETRIEVAL-SYNTHESIS-V1 stream (registers 11.277–11.280) is now committed: `orchestrator/api/{chat_retrieval,ui}.py` (GRAPH RELATIONS `[G#]`→proving `[S#]`, WILDCARD profile-atom map-door), determinism tests, the ELITE plan/report/5 work-logs, register rows 11.277–280, scaffold decls. Boundary forensically separated — EXCLUDED + left uncommitted: `scripts/verify_final_state.py` (pre-existing re-fire-semantics change, NOT ELITE-declared), the frontend-v2 UI work (mine), `Polymath_librarian_architecture_checklist.md` (mine). ELITE code + its two new determinism tests green in situ; the one red (`test_live_artifact_tasks[brainrot_transform]`) is a PRE-EXISTING missing-corpus (`ecom-meta-v1`) data gap, not in the diff.

**LIBRARIAN REBASED onto `4d95da2` → tip `8f044b2` (branch `librarian/retrieval-architecture`, worktree `pmv4-librarian`), UNMERGED.** The six foundation slices were rebased off cf1ee4f onto the ELITE base; the register-number collision is CLEARED by renumbering the librarian rows **11.277–282 → 11.281–286** (register rows AND commit subjects), monotonic after ELITE's 11.280. 39/39 slice unit tests green on the new base; repo_guard/wiki_worm ok, bundle READY. Slices: **11.281** P4 canonical-selection guard · **11.282** P4a SurfaceRegistry · **11.283** P4a profile_atom DAG wiring · **11.284** P1/P2 lifecycle schema (migration `0065`, UNAPPLIED) · **11.285** P1/P2 lifecycle writer · **11.286** P1/P2 projection reconcile.

**Topology:** `cf1ee4f → bf7b517 (continuity) → 4d95da2 (ELITE base)`; librarian `8f044b2` branches off `4d95da2`.

**OPEN GATES.** (1) Migration `0065` AUTHORED but UNAPPLIED — gate before any lifecycle-writer live path (queue L3). (2) Live-qualification queue L1–L5 (apply 0065, wire projectors, bounce, adversarial acceptance) — promotes the six slices to DONE_AND_PROVEN; not run. (3) Forensic cinema hold still in force. (4) LEFTOVERS uncommitted on `production` (do NOT clobber, do NOT `git add -A`): the frontend-v2 UI work + `Polymath_librarian_architecture_checklist.md` (mine) and `scripts/verify_final_state.py` (unattributed pre-existing) — `repo_guard` RED on `production` from these, PRE-EXISTING. (The register collision, prior gate 5, is RESOLVED.)

**NEXT ACTION:** the authoritative base exists. Per the owner, INSPECT the post-ELITE retrieval code (`orchestrator/api/{chat_retrieval,ui}.py` as they now stand at `4d95da2`) BEFORE inventing P5/P6 structures, then build the real core IN ORDER — `P5 Profile Scout → P6 subquery provenance → P10 evidence-driven resolution → P11 profile-yield receipt → integration → ONE live E2E (L1–L5)`. Do NOT yet: apply 0065, bounce the fleet for librarian activation, run L1–L5, or refactor graph traversal (preserve the qualified hop-1 path). Build P5/P6 in the `pmv4-librarian` worktree off `8f044b2`.
