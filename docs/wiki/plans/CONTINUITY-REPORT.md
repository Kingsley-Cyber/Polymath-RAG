...
## CURRENT — 2026-09-18 — CONSTRAINT-AWARE-RETRIEVAL-V1 CA0–CA3 DEPLOYED + LIVE; named-source ranking FIXED (pmap MRR 0.25→1.0); next = CA4 (evidence roles, owner-gated)

**Repository State** — branch `production`, HEAD **400c5fb**, tree CLEAN. Worktree `pmv4-constraint` (branch `constraint/aware-retrieval`) == production. Stash `49f57e0c` (frontend/ELITE) still HELD. Fleet BOUNCED on the CA bundle (supervisor 65431, 11 workers healthy/one bundle, `/ready` true). **Flag `POLYMATH_CHAT_CONSTRAINT_ALIGN=1` now LIVE in `.env`** (plus scout/expansion/resolution/intent-policy on).

**Active Mission** — CONSTRAINT-AWARE-RETRIEVAL-V1 (admitted `9d89dd8`, binding ADMISSION in `docs/wiki/plans/CONSTRAINT-AWARE-RETRIEVAL-V1.md`). CA0–CA3 DONE + deployed + live-proven; **CA4 (evidence-role grades DIRECT/PARTIAL/RELATED + need→evidence synthesis threading) NOT started, owner-gated.**
...
**Next Action** — CA4 (owner-gated): per-evidence `support_role` DIRECT/PARTIAL/RELATED from multi-signal (need satisfaction + constraint satisfaction + relevance + coverage, NOT rerank bands) + thread need→evidence→support_role into `assemble_evidence_bundle` (reuse `POLYMATH_CHAT_SYNTH_ROLES`); RELATED usable, SYNTHETIC_INSIGHT distinct; resolves the unsupported-hallucination gate (grade RELATED/none, state the gap). Then CA5 full 64×4 qualification vs baseline + the 6 admission acceptance cases (ONE final run).
