...
## CURRENT — 2026-09-18 — CONSTRAINT-AWARE-RETRIEVAL-V1 CA0–CA4 DEPLOYED + LIVE; named-source ranking FIXED + evidence roles/epistemic gate live; next = CA5 (full qualification, owner-gated)

**Repository State** — branch `production`, HEAD **bebda64**, tree CLEAN. Worktree `pmv4-constraint` (branch `constraint/aware-retrieval`) == production. Stash `49f57e0c` (frontend/ELITE) still HELD. Fleet BOUNCED (supervisor 68668, 11 workers healthy/one bundle, `/ready` true). **Flags now LIVE in `.env`: `POLYMATH_CHAT_CONSTRAINT_ALIGN=1` (CA3) + `POLYMATH_CHAT_EVIDENCE_ROLES=1` (CA4)** (plus scout/expansion/resolution/intent-policy on).

**Active Mission** — CONSTRAINT-AWARE-RETRIEVAL-V1 (admitted `9d89dd8`). CA0–CA4 DONE + deployed + live-proven; **CA5 (full 64×4 qualification vs BASELINE-cinema-2026-09-18 + the 6 admission acceptance cases, ONE final run → close-out) NOT started, owner-gated.**

**CA4 live-proven (register 11.309)** — evidence-role grading (`grade_evidence`: DIRECT=user-need+relevant+HARD-source, PARTIAL=need+relevant non-source, RELATED=bridge-only/not-relevant) + a multi-signal answerability gate in `render_answer` (bundle `epistemic.establishes_need=False` → states the gap even if lexical coverage passes; byte-identical without the key). LIVE: "In Murch's book" → 3 DIRECT / 7 PARTIAL / 5 RELATED (the epistemic portfolio); supported queries → DIRECT grades, answer normally; the 4 unsupported → state the gap (ranked=0 this run via compiler-decline; the CA4 gate is the safety net for the spurious-PRIMARY case). Flag `POLYMATH_CHAT_EVIDENCE_ROLES`.
...
**Next Action** — CA5 (owner-gated, CLOSE-OUT): ONE full 64×4 qualification (all CA flags on) vs `BASELINE-cinema-2026-09-18.summary.json` + the 6 admission acceptance cases. Assert: named-source single-target MRR ≥0.80 (pmap now 1.0), success@10 ≥0.90 all modes, unsupported-hallucination=0 (CA4 gate), provenance/q0 100%, no regression on the 5 aligned + 59 unchanged queries. **Before CA5: close the harness abstention-detection gap** — `harness.py:124` insufficient-detection does NOT recognize the standard abstention "I don't have enough grounded evidence…", so a CA4 gate-abstention with ranked>0 (spurious-PRIMARY case) would be miscounted as hallucination; add the actual `ABSTENTION_MESSAGE` phrase to the detection (evaluator decline-recognition completeness — NOT gate-lowering; document transparently). Persist the passing artifact; then close-out (checklist/ledger/CONTINUITY final numbers; stash restore is a SEPARATE owner decision — the frontend stream, not part of this mission).

**Do Not Do** — start CA4 before owner "go"; numeric source boost; RRF/rerank weight tuning; gold change; second engine; remove cross-encoder; Graph multi-hop; `git push`; restore the stash yet. Detection false-positives on "the X system/method" + bare author names are HARMLESS now (resolver leaves them unresolved) — tighten in CA4 if desired, not urgent.
