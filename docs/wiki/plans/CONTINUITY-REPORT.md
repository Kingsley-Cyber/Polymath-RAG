...
## CURRENT — 2026-09-19 — **WLK2C-RETRIEVAL-LINEAGE-V1 = V1 baseline, COMPLETE + causally PROVEN** (merged, flags OFF, NOT qualified as final); **LATENT-QUERY-FUSION-V2 ADMITTED + F1+F2+F3 DONE + PROVEN (registers 11.327–330; RankedLane + weighted RRF + the real selection seam, all worktree, flag-off no-op) — checkpoint answer YES (alters candidate survival at the real truncation seam, strict no-op disabled); F4 (merge+bounce+A/B) held for separate owner authorization**; CA0–CA5 COMPLETE; WLK2A/2B done

**Repository State** — branch `production`, HEAD **575f228** (F1–F4step6; F1–F4step1 merged at `ae10f5a` + steps 2–6 on top), tree CLEAN (`.env` gitignored). **V2 IS LIVE (PROVISIONAL — CA5 gate running).** `.env` now `POLYMATH_CHAT_LATENT_FUSION=1` + `LATENT_SELECTION=1` + `BRIDGE_COMPILER=1`; fleet BOUNCED onto step-6 code, orchestrator pid env carries all three flags, sidecars up, `/ready` true. A/B (10×3×3 repeats) proved **V2 materially healthier than V1** (deep_final_reach 0.667 vs 0.467, V2≥V1 every run; q0 0.990; latency 12896<15214). Step 6 = NO tuning, config-driven surface added (`FusionWeights.from_env`/`POLYMATH_FUSION_*`, defaults=validated). **F4 step 7 CA5 64×4 (256 turns via live /chat/stream) IN PROGRESS** — must hold halluc=0 / q0_preserved=1 / provenance=1 / no material success@10 or single-target-MRR regression vs `CA5-cinema-2026-09-18` (FAST 1.0/0.946, others 0.983/0.839; known flag resolution_trigger 0.25). **If CA5 regresses → REVERT the 3 .env flags to 0 + bounce (V2 back to flagged-off proven capability).** Worktree `pmv4-fusion` retained (merged; prune later). WLK2C worktree `pmv4-wlk2c` (branch `wlk2c/retrieval-lineage`) MERGED. Stash `49f57e0c` (frontend/ELITE) UNTOUCHED. **Fleet: on d904154, WLK2C flags OFF + `POLYMATH_CHAT_LATENT_FUSION` unset/OFF, 11 healthy/one bundle → pre-WLK2C no-op behavior; F1/F2 are worktree-only + pure, no bounce.** LIVE flags: CA3/CA4 + scout/expansion/resolution/intent-policy (all pre-WLK2C, on). WLK2C V1 is the flagged BASELINE for A/B against V2. **Pre-existing (NOT mine): `tests/determinism/test_chat_modes.py:327` (WILDCARD sweep `sweep_done_before_core` TIMING assertion) fails 3/3 on unmodified d904154 — a timing flake, unrelated to F1/F2/F3.**
...
**Next Action** — LATENT-QUERY-FUSION-V2 (plan-of-record `docs/wiki/plans/LATENT-QUERY-FUSION-V2.md`) on worktree `pmv4-fusion` (`831a2e5`). **DONE + PROVEN: F1 (11.328, `a6dc4a9`)** — pure `shared/polymath_shared/ranked_lane.py` + flag-gated capture + `trace['ranked_lanes']` in `retrieve_candidates`; UNIT+WORKTREE_INTEGRATION (51/51), flag-off byte-identical (`on_union==off_union`), NO selection change. **F2 (11.329, `831a2e5`)** — pure `shared/polymath_shared/ranked_fusion.py`: `fuse_ranked_lanes` = weighted RRF (`Σ weight(lineage_class)/(k+local_rank)`) + bounded local-winner preservation; UNIT_PROVEN (8/8) incl. a bridge's local rank-0 surviving a cap of 3 (truncated at preserve_top_n=0); candidate ordering only, no seating. **F3 (11.330, `e042780`) DONE + PROVEN** — `candidate_engine.py:1066`, flag-on, `_latent_fused_union` replaces the flatten `union = fused[:merged_candidate_max]` with the F2 fused ordering + bounded preservation (reuses the same CandidateEvidence, one physical candidate/chunk, cap unchanged, fail-open); `SubQuery.origin` added; `_apply_cap` strict ceiling. Owner constraints honored (flag-off exact / no tuning / cap preserved / origin from existing provenance / C4→C5→CA4 unchanged). Owner checklist all green (68 V2 + 51 consumer tests): flag-off union == flatten prefix; flag-on q0+bridge survival, dedup, lineage attached, cap unchanged, preserve_top_n=0 degenerates, fail-open; **THE actual failure proven — 100 q0 flood + bridge rank-1 expert, cap 50 → flatten truncates, V2 preserves inside the cap — AND end-to-end via retrieve_candidates**. CHECKPOINT ANSWER = **YES**. **NEXT = F4 (LIVE CUTOVER — HELD for separate owner authorization).** F4 = (1) orchestrator populates `SubQuery.origin` from the plan's existing subquery provenance (chat_retrieval `sub_specs` 4-tuple → carry origin; live-only, not worktree-provable per the editable-.pth); (2) merge `fusion/latent-query` → production; (3) port-gated bounce; (4) A/B (V2 vs WLK2C V1 vs pre-WLK2C) on WLK-10 + 4-mode survival + main harness + CA5 64×4; (5) calibrate weights + preserve_top_n FROM DATA. wc01/wc05/wc07 = DIAGNOSTIC PROBES, not optimization targets. **Do NOT merge/bounce/A-B until owner authorizes F4. Do NOT re-run/reopen WLK2C/C7.** SEPARATE owner decisions: WLK1/WLK3; restore stash `49f57e0c`.

**Do Not Do** — (V2 F4-HOLD) merge the `fusion/latent-query` worktree, bounce the fleet, flip `POLYMATH_CHAT_LATENT_FUSION` on live, or run WLK-10 / survival / CA5 A/B — ALL held until the owner authorizes F4. (V2) rewrite `CandidateEngine` unless investigation PROVES it necessary (F3 used a bounded l.1066 adapter, no rewrite); hardcode wc01/wc05/wc07 behavior; blindly increase global K; tune/freeze lane weights or preserve_top_n before F4 measurement (F3 used F2 defaults); give bridge-local winners AUTOMATIC final seats (they still pass C4/C5); collapse many-to-one lineage before C4; expand `merged_candidate_max`. **Re-run or reopen the completed WLK2C/C7 work** (V1 is a frozen baseline). (Standing, WLK2C) `max(q0,bridge)`; per-candidate model call; lower the global rerank floor; the WLK2C locks (grounding split, anti-hijack) stay. (Standing) reopen CA0–CA5; second retrieval engine; remove the cross-encoder; Graph multi-hop; `git push`; restore the stash unilaterally; regenerate the frozen 2026-09-18 baseline artifacts.
