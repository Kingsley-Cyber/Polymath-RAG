...
- **current implementation slice** — **S0 (admission) DONE** (11.129), **S1 (deterministic ParentSkeleton) DONE** (11.130: `parent_skeleton.py`, 13 pins), **S2 (parent-map compiler) DONE** (11.131: `map_compiler.py` — tolerant DSL, strict identity, exact partial recovery, skeleton identifiers, 15 pins). **S3 (token packer / capacity model) is the next executable slice.** Slice ledger: S0…S16 in the plan §40.
...
- **exact next action** — re-read plan §40 slice **S3**, admit it in a work-log, then build `shared/polymath_shared/document_profile/map_batches.py` + `tests/determinism/test_*` (input-token estimate; billed/visible output EMAs SEEDED from the measured baseline 135.7 input / 87 billed / ~29 visible tokens per parent; the combined global-profile+MAP capacity formula of §13.2; mapping-only capacity — ~60 target, 40 proven, do not pack to the ~74 edge; the ≤ 15k-tokens/request TPM guard; deterministic batch manifests over the S1 skeletons). Do not hard-code capacity forever — update from receipts. Composes with S1 `build_parent_skeletons` and S2 `compile_maps`.
