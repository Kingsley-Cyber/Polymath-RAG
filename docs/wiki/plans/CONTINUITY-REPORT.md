...
- **exact next action** — re-read plan §40 slice **S4** (+ §20), admit it in a work-log, then add `stores/postgres/migrations/<next>_document_parent_maps.sql`: the `document_parent_map_batches` table (resumable substage — status pending/leased/partial/done/error, expected/valid counts, `alias_manifest` JSONB, input/raw hashes, lease owner + expiry) and `document_parent_maps` (final authoritative map — routing_signature, semantic_hooks JSONB, exact_identifiers JSONB, source/map hashes, `active`, UNIQUE active per doc_id+parent_id+map_contract) plus the parent-exclusion record; idempotent insert/update; NO worker yet. Migrations are append-only and idempotent (`IF NOT EXISTS`); a migration change needs a work-log (repo_guard companion rule). The rows persist exactly the S2 `CompiledMap` and the S3 `MapBatch` manifests.
