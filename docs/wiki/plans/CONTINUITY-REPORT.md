...
## CURRENT — 2026-09-18 — CONSTRAINT-AWARE-RETRIEVAL-V1 **COMPLETE**; WLK2B → NULL; WLK2A → Retrieval-Lineage VALIDATED; **WLK2C-RETRIEVAL-LINEAGE-V1 ADMITTED (owner /goal) — executing C0→C7 on worktree `wlk2c/retrieval-lineage`**; WLK1/WLK3 owner-gated

**Repository State** — branch `production`, HEAD **d155fcd** (+ this WLK2A-findings commit, eval/+docs only), tree CLEAN. Worktree `pmv4-constraint` (branch `constraint/aware-retrieval`) synced. Stash `49f57e0c` (frontend/ELITE) UNTOUCHED — restoring it is a SEPARATE owner decision. Fleet on the CA0–CA4 bundle (11 workers healthy/one bundle, `/ready` true; the WLK commits — eval/docs only — touch NO fleet code → NO bounce, committed==live; **WLK2B + WLK2A shipped no production change**). **LIVE flags: `POLYMATH_CHAT_CONSTRAINT_ALIGN=1` (CA3) + `POLYMATH_CHAT_EVIDENCE_ROLES=1` (CA4)** + scout/expansion/resolution/intent-policy.

**Active Mission** — (1) CONSTRAINT-AWARE-RETRIEVAL-V1 (admitted `9d89dd8`) **COMPLETE / DONE_AND_PROVEN** (registers 11.305–311). (2) **WILDCARD-LATENT-KNOWLEDGE investigation** (registers 11.312–314) — benchmark + 40-case survival trace, baselines FROZEN. **WLK2B (rerank-survival) → NULL (11.313)**: deep chunks reaching the judge are sub-floor, winners not redundant. **WLK2A (latent activation) INVESTIGATED → Retrieval-Lineage architecture VALIDATED (11.314): no production change yet** — reranker judges every candidate against PRIMARY q0 only (the bridge is discarded); the existing PROFILE-expansion bridges are misdirected. DECISIVE bridge-as-primary test: running full retrieval+rerank with a GOOD bridge surfaces the right expert passages and seats them in FINAL (wc01 expert-in-union 2→91, best rerank −6.58→+5.52, in-final 0→10; wc05 +5.87/5; wc07 +6.07/8) — corrects the ceiling read (wc01/wc05 are BRIDGE-limited, NOT chunk-quality; the corpus has strong chunks q0-driven retrieval wasn't surfacing). Mechanism proven; crux = bridge generation. Ready to admit an implementation plan; owner picks the bridge source. WLK1/WLK3 owner-gated, NONE started. NEW axis (discovery → survival → utilization), distinct from CA0–CA5 — do NOT reopen CA0–CA5.
...
**Next Action** — **execute WLK2C** (ADMITTED; plan-of-record `docs/wiki/plans/WLK2C-RETRIEVAL-LINEAGE-V1.md`). Owner chose a TIERED bridge policy: reuse existing good subquery → derive from a GRAPH path → bounded concept-bridge COMPILER (one structured LLM call/eligible query, ≤3–4 grounded bridges) → else no latent expansion. Role admission DIRECT/COMPLEMENTARY/DIVERGENT (q0 PRIMARY, NOT max; floor unchanged; extends CA4). In-flight on worktree `wlk2c/retrieval-lineage`. Execute narrowly: **C0** lineage fields (shared/ `retrieval_lineage.py` + candidate_engine, UNIT) → C1 reuse subquery/graph bridges → C2 bounded compiler (flag) → C3 deepen w/ origin_query → C4 rerank vs q0+origin → C5 bounded roles → C6 observability → **C7 merge+port-gated-bounce+qual** (WLK-10 + survival + main harness + CA5 64×4 vs frozen baselines). Each flag default-off. **Separate/after:** Scout coverage for wc03/wc10; WLK1 routing (#2); WLK3 synthesis-spend (#4). SEPARATE owner decision: restore the frontend/ELITE stash `49f57e0c` (`git stash pop`, never force).

**Do Not Do** — (WLK2C) use `max(q0_score, bridge_score)` as the final score (role admission only; q0 primary); let a bridge override q0 or rescue arbitrary low-relevance material; add a per-candidate model call; **bounce the fleet mid-mission** (one bounce at C7 only); lower the global rerank floor; hard-code FACS/Laban/Murch/Timing/cinema/benchmark concepts; bundle wc03/wc10 nomination or WLK1 routing into WLK2C. (Standing) **re-attempt a WLK2B portfolio-survival mechanism** (NULL) or tune the floor for wc01/05/07; **regenerate the frozen `BASELINE`/`SURVIVAL`/`WLK2B-FORENSICS`/`WLK2A-*`-2026-09-18 artifacts** (immutable); reopen CA0–CA5; second retrieval engine; remove the cross-encoder; Graph multi-hop; `git push`; restore the stash unilaterally.
