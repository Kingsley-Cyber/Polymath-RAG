...
## CURRENT — 2026-09-19 — **WLK2C-RETRIEVAL-LINEAGE-V1 = V1 baseline, COMPLETE + causally PROVEN** (merged, flags OFF, NOT qualified as final); **LATENT-QUERY-FUSION-V2 ADMITTED + F1 DONE (register 11.327–328; RankedLane representation, flag-off no-op proven) — F2 (lineage-aware weighted RRF) NEXT**; CA0–CA5 COMPLETE; WLK2A/2B done

**Repository State** — branch `production`, HEAD **0887188** (V2 admission docs-only, on d904154 = WLK2C merged 86196eb + C7 pool-fix), tree CLEAN. **V2 work lives on worktree `pmv4-fusion` (branch `fusion/latent-query`), HEAD `a6dc4a9` = F1 committed, UNMERGED (merges at F4).** WLK2C worktree `pmv4-wlk2c` (branch `wlk2c/retrieval-lineage`) MERGED. Stash `49f57e0c` (frontend/ELITE) UNTOUCHED. **Fleet: on d904154, WLK2C flags OFF + `POLYMATH_CHAT_LATENT_FUSION` unset/OFF, 11 healthy/one bundle → pre-WLK2C no-op behavior; F1 is worktree-only, no bounce.** LIVE flags: CA3/CA4 + scout/expansion/resolution/intent-policy (all pre-WLK2C, on). WLK2C V1 is the flagged BASELINE for A/B against V2. **Pre-existing (NOT mine): `tests/determinism/test_chat_modes.py:327` (WILDCARD sweep `sweep_done_before_core` TIMING assertion) fails 3/3 on unmodified d904154 — a timing flake, unrelated to F1.**
...
**Next Action** — continue LATENT-QUERY-FUSION-V2 (plan-of-record `docs/wiki/plans/LATENT-QUERY-FUSION-V2.md`) on worktree `pmv4-fusion`. **DONE: F1 (register 11.328, `a6dc4a9`)** — pure `shared/polymath_shared/ranked_lane.py` (`RankedLane`/`build_ranked_lanes`/`lane_memberships`/`ranked_lanes_receipt`) + a flag-gated capture + `trace['ranked_lanes']` in `candidate_engine.retrieve_candidates`; UNIT+WORKTREE_INTEGRATION proven (51/51), flag-off byte-identical (`on_union==off_union`), NO selection change. **NEXT = F2** — deterministic lineage-aware **weighted RRF** over the RankedLanes (`Σ lane_weight/(k+local_rank_i)`) with **bounded local-winner preservation** (each query's top-N reserved through `merged_candidate_max`/`judged_prefix`); pure + unit-testable in the worktree; weights (q0 1.0 / COMPLEMENTARY 0.6 / DIVERGENT 0.3 / GRAPH 0.5) PROVISIONAL, measured at F4, NOT frozen. Then **F3** feed the fused set to the existing C4→C5→CA4→synthesis (live-only, orchestrator wiring — enrich `origin` from plan provenance here); **F4** merge+bounce+A/B (V2 vs V1 vs pre-WLK2C) on WLK-10 + survival + main harness + CA5 64×4. Investigation DONE (root cause = flatten-too-early; see Active Mission). wc01/wc05/wc07 = DIAGNOSTIC PROBES, not optimization targets. **Do NOT re-run/reopen WLK2C/C7.** SEPARATE owner decisions: WLK1/WLK3; restore stash `49f57e0c`.

**Do Not Do** — (V2) rewrite `CandidateEngine` unless investigation PROVES it necessary; hardcode wc01/wc05/wc07 behavior; blindly increase global K; freeze speculative lane weights before measurement; give bridge-local winners AUTOMATIC final seats (they still pass C4/C5); collapse many-to-one lineage before C4; make F1 change selection (representation/observability only). **Re-run or reopen the completed WLK2C/C7 work** (V1 is a frozen baseline). (Standing, WLK2C) `max(q0,bridge)`; per-candidate model call; lower the global rerank floor; the WLK2C locks (grounding split, anti-hijack) stay. (Standing) reopen CA0–CA5; second retrieval engine; remove the cross-encoder; Graph multi-hop; `git push`; restore the stash unilaterally; regenerate the frozen 2026-09-18 baseline artifacts.
