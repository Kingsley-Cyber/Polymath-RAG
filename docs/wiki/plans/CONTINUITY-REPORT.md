...
## LIVE DEPLOY CHECKPOINT — 2026-09-18 — Librarian MERGED to production + 0065 applied + fleet bounced + scout/P6 LIVE-PROVEN; qualification underway
...
**Qualification (mission §10–§27)** — harness + gold set in `eval/librarian_qualification/` (`harness.py` real `/chat/stream` probe + IR metrics; `build_gold.py` → 64 cinema queries incl. 10 sensitivity pairs + 4 unsupported; `summarize.py` acceptance floors; `cinema_docmap.json`). Baseline FAST+HYBRID running; early gold-hit rate 8/8. Primary metric = success@10 (gold_hit) + MRR (gold is acceptable-source sets; coverage-fraction is secondary).

**REMAINING to DONE_AND_PROVEN** — (1) finish + analyze the baseline, repair real retrieval misses; (2) **P11 profile-expansion + yield** live wiring — add PROFILE-origin subqueries from scout nominations in `_compile_chat_plan` (`ui.py`), they flow to retrieval at `ui.py:2942`, yield reads `_aspects` (`aspect_final[qid]`) at `ui.py:2944` — currently yield is 0/0 (scout only conditions the compiler via titles; no PROFILE-origin subqueries yet); (3) **P10 resolution** live wiring (bounded round via `chat_retrieve_v2`, `evidence_resolution.plan_resolution_round`); (4) GRAPH + WILDCARD mode qualification; (5) restore the stash. Flag-gate + fail-open all new wiring; bounce port-gated.
