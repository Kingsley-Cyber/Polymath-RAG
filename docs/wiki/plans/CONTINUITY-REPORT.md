...
## CURRENT — 2026-09-18 — CONSTRAINT-AWARE-RETRIEVAL-V1 **COMPLETE**; WILDCARD-LATENT-KNOWLEDGE-10 benchmark COMMITTED + baselines FROZEN; WLK surgical slices queued (owner-gated)

**Repository State** — branch `production`, HEAD **76ae586**, tree CLEAN. Worktree `pmv4-constraint` (branch `constraint/aware-retrieval`) synced to `76ae586`. Stash `49f57e0c` (frontend/ELITE) UNTOUCHED — restoring it is a SEPARATE owner decision. Fleet on the CA0–CA4 bundle (11 workers healthy/one bundle, `/ready` true; the eval-only WLK commits `859ae34/c3b5616/76ae586` touch NO fleet code → no bounce, committed==live). **LIVE flags: `POLYMATH_CHAT_CONSTRAINT_ALIGN=1` (CA3) + `POLYMATH_CHAT_EVIDENCE_ROLES=1` (CA4)** + scout/expansion/resolution/intent-policy.

**Active Mission** — (1) CONSTRAINT-AWARE-RETRIEVAL-V1 (admitted `9d89dd8`) **COMPLETE / DONE_AND_PROVEN** (registers 11.305–311). (2) **WILDCARD-LATENT-KNOWLEDGE investigation** (register 11.312) — a new latent-knowledge retrieval benchmark + a 40-case 4-mode survival trace, baselines FROZEN, four surgical WLK slices identified, NONE started (owner-gated). This is a NEW axis (discovery → survival → utilization), distinct from CA0–CA5 (evidence-correctness) — do NOT reopen CA0–CA5.
...
**Next Action** — WLK surgical slices (owner-gated; each reruns the FROZEN benchmark vs `BASELINE`/`SURVIVAL`-2026-09-18, one fix must not degrade another). Recommended order: **1. WLK2B** (shared rerank-survival of high-value deep material — the top lever; targets the 3 uniform-across-mode deaths Ekman/Timing/Laban, helps all four modes; success = deep-survival@final 0.70→~0.90 with no loss to healthy cases). 2. WLK2A (nomination depth — #3/#6/#10; may need no production change). 3. WLK1 (imperative creative-rewrite routing — #2 `TRANSFORM_USER_CONTENT` misclassification). 4. WLK3 (synthesis-spend of surviving RELATED/PARTIAL — #4). CONSTRAINT-AWARE has no open action. SEPARATE owner decision: restore the frontend/ELITE stash `49f57e0c` (`git stash pop`, never force).

**Do Not Do** — start ANY WLK slice before owner "go"; **regenerate the frozen `BASELINE`/`SURVIVAL`-2026-09-18 artifacts** (immutable — new slices write NEW artifacts); reopen CA0–CA5; numeric source boost; RRF/rerank weight tuning; gold change; second retrieval engine; remove the cross-encoder; Graph multi-hop; `git push`; restore the stash unilaterally.
