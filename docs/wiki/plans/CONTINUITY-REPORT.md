...
- **Fleet:** 26 healthy / 13 types; 25 workers on bundle `15247f145355` (the 22:35 bounce) and `adapter_step` on `785fd9efedf4`: the
  supervisor restarted it at 05:19Z after the Docker crash while an untracked file sat in the main checkout (`tree_dirty`); same code,
  the next bounce unifies it — not a fence fault. `/ready` true. Docker Desktop was restarted at ~05:22Z (see Runtime).
- **Live `.env` (gitignored):** unchanged (SEEALSO-BLEND, D1, DOC-STEER on; R8 on unless `POLYMATH_ACQUISITION=0`; OpenCLI
  `/opt/homebrew/bin/opencli` 1.8.6, bridge connected).
...
- **Order (roadmap §3):** … K1 ✔ → K1b ✔ → 6d AUTORESEARCH ✔ → the worker pack → C1 → C2 → C3 → L5 → …
- Document RAG stays paused (S5–S7, S9).

### Completed Since Last Bootstrap (11.494–11.495)
- **11.495 R7:** the one e2e run + the harness smokes; gap rows A-09..A-14, S-16..S-20, O-06..O-08, H-08..H-10; S-13 corrected.
- **11.494 R6:** merged + bounced (the owner's Run button); the Hermes skill copy; live check 14/14.

### Current Contract State
- **Knowledge scope (K1):** a new search function, or a new call of a scoped search, must thread the request's scope
  (`**scope_kwargs(scope)`); the caller pin in `tests/determinism/test_knowledge_scope.py` fails otherwise (it walks
...
- **Lane G (chat):** the global door + the SEE ALSO blend (knobs `_DOCS / _ITEMS / _CHILDREN / _ALPHA` = 4 / 4 / 4 / 0.7)
  + DOC-STEER in every mode that opens lane G (HYBRID with nominated documents, GRAPH, WILDCARD); receipt
  `query_receipts.meta.retrieval_trace.seealso_fanout`. `/retrieve` does not apply the skeleton routes (lane G off there).
...
- 11.494 / 11.495: no code changed; ADAPTER_RUNTIME, ECOMMERCE_ENGINE, RESEARCH_ACQUISITION TESTED_UNCHANGED (their defects are rows).
- DEFERRED as before: the fleet-DB test files; nothing BLOCKED.

### Proof Status
- **11.495 R7:** EXECUTED live. R8 LIVE_PATH_PROVEN (102 `research_acquire` calls from an MCP client that runs no OpenCLI: walls
  refunded, per-comment dates, the page-date bound). R1 LIVE_PATH_PROVEN (TikTok + Instagram comments admitted as field evidence).
- **11.494 R6:** EXECUTED live (fast-forward + READY; live check 14/14).
- 11.490–11.493: live since 11.494. Earlier: K1 / K1b / D1 LIVE_PATH_PROVEN; the SEE ALSO blend + DOC-STEER wait for one GRAPH /
  WILDCARD chat. INVALIDATED: none.
...
- Exclude the fleet-DB files (`test_query_receipts.py`, `test_incremental_census.py`,
  ...
  `test_query_receipts.py`'s pure tests can run with `-k "not test_record_then_read_back_through_both_surfaces"` (the only
  test that opens its hard-coded fleet DSN).
- **Merge → bounce window (11.479):** a new orchestrator file imported lazily can meet an old in-memory module. Guard
  ...
  `orchestrator.log` for a new Traceback. K1 adds no such import (new symbols live only in the new `code/scope.py` and in
  `query_receipts`, which nothing else imports from).
- Known pre-existing failures: `test_chat_modes::test_wildcard_sweep_overlaps…`, `test_chat_runtime::test_compiler_on…`,
  `test_synthesis_attempt_telemetry::…` (order-dependent), `test_query_receipts::test_all_three_query_handlers…` (pins 2
  chat receipt writers; production has 4 since /chat/evidence). CI `determinism` = 13 known failures (on `9fc0956f`); CI
  `contracts` green since 11.476.
- **Docker Desktop crash (2026-09-26 05:15Z):** the VM powered off ("com.docker.virtualization: process terminated unexpectedly") and
  Docker showed Quit / Reset. Recovery = `osascript -e 'quit app "Docker"'`, quit the "Docker Desktop" window and its error dialog,
  `open -a Docker`; the stores return healthy in about a minute and the fleet reconnects on its own. NEVER "Reset to factory
  defaults" (it deletes every container and volume). `/ready` does not check Postgres (O-06): check the DB directly.
- An untracked file in the MAIN checkout makes any worker the supervisor restarts report `tree_dirty` (a second bundle hash, same
  code): keep run artifacts outside the main checkout, or commit them before a bounce.
- `codex exec` waits on stdin: give it `< /dev/null`. The ChatGPT app bundles a newer `codex`
  (`/Applications/ChatGPT.app/Contents/Resources/codex`) than Homebrew's.

### Working Tree
- Main checkout: clean (`production`) after the close-out commit.

### Tooling State
- graft refreshed at `b3daddd0`; guards 0 on the close-out (preflight, repo_guard, wiki_worm); bundle_integrity READY.
...
- Never let a consumer that sends a scope accept an unconfirmed reply (K1b); never roll the orchestrator back without
  `adapter_step` (a K1b `adapter_step` refuses every Trail lane of an older orchestrator).
...
- Never click Docker Desktop's "Reset to factory defaults"; never clear a site's human check yourself (the owner does, in the host
  browser); never present a concept contested by an existing product as an opportunity.

### Live Qualification Queue
- The SEE ALSO blend + DOC-STEER first live receipt (the owner's next GRAPH / WILDCARD chat).
- A fix to A-09 / A-10 / S-16 / S-17 is proven only by a new governed run (the owner's word; R7 used the one authorized run).
...
- Gap rows O-01..O-08, T-01, T-02, D-03..D-09, A-01..A-14, S-09..S-20, H-08..H-10: open, not scheduled. GNN retrieval stays experimental.

## PRIOR — 2026-09-25 (autoresearch R1 + R8 + the audit's fixes built on the branch; the owner's Run button next) — **the handoff before the merge: R2–R5, R8, R1 and the audit fixes (11.490–11.493) on `feat/autoresearch-sources-harness`.** Superseded by the block above; its Next Action list (Run button → skill copy + live check → the R7 run → smokes → close-out) was executed.
...
5. **Atom repair: DONE 2026-09-24 (owner: "go ahead with the atom repair"; register 11.445).**
   - Cinema atoms went 609 → 2,453 active (THEORY / CONCEPT / SEEALSO about 10 per book again, beside the vNext latent kinds),
     reconciled with Qdrant.
   - Supersession is now family-scoped (base vs vNext), so a fresh profile of one family can never deactivate the other's atoms.
   - The worker embeds atoms in the collection's query mode.
   - Rollback: `scripts/repair_profile_atoms.py --corpus cinema --restore ~/PolymathBackups/atom-repair-2026-09-24` + tag
     `pre-atom-repair-merge`.
6. **S4 compiler contract: BUILT + MEASURED 2026-09-24, flag OFF** (owner: "yes start S4"; register 11.449; work-log
   `2026-09-24-s4-compiler-contract.md`).
   - `POLYMATH_CHAT_COMPILER_CONTRACT=1` makes the one compiler call also write:
     - the learning need;
     - a purpose + evidence need per query;
     - inquiry dimensions and ≤ 3 synthesis targets;
     - ≤ 3 bridges from the Scout's matched profile items. They pass the retired bridge call's own admission, so that
       second model call is gone.
   - D2: knowledge questions always retrieve.
   - Measured (compiler calls only, 3 rounds): compile median 2.98 s vs 3.03 s. Synthesis turns are faster (no second
     call), factual turns +0.7 s. Turns with bridges 11/20 both. Fallback 1/60 vs 0/60, inside the live 4.0% baseline.
   - Flip only on the owner's word, after a flagged live check (S9). Nothing reads the new fields yet (S7 judge, S8 answer).
   - A bug fix ships either way: the instruction-word filter matched substrings ("information" holds "format") and
     silently dropped topical planner queries.
   - Test-hygiene incident: a stopped full-suite run left 7 `census_probe_*` runs (57 attempts, corpus `census-probe`,
     1 cursor) in the fleet DB. **Cleaned up by the owner 2026-09-23 night** with the module's own teardown purge
     (`test_incremental_census._purge_probe_rows`); 0 rows left, verified.
...
- HEAD = this documents commit, on top of merge `6f18017` (S8 synthesis contract, register 11.450; merged by the owner) and
  ...
  - Rollback tags (local): `pre-s8-merge` → `fea18f9`, `pre-s4-merge` → `ea950a2`, `pre-probe-gate-merge` → `0e0978b`, `pre-atom-repair-merge` → `903cb89`, `pre-v11-groq-merge` → `0e57c46`, `pre-skeleton-routes-merge` → `4ab046e`.
- **Deploy (2026-09-23 night, done by the owner):** the permission classifier denied the agent's bounces and the S8 merge,
  so the owner ran them.
  - Bounce 22:26 MDT loaded S4.
  - Merge `6f18017` 23:07.
  - Bounce 23:29 loaded S8.
  - Verified: 24 / 13 / ONE bundle `3b8b06f49864`, `/ready` true. Neither new switch is set in the orchestrator process.
  - Worktrees `pmv4-probe-routes` / `pmv4-groq` / `pmv4-s4` / `pmv4-s8` are removed after merge; their branches are merged.
  - Unmerged worktree branches (not tonight's): `review/m1-reproductions` (+1), `handoff/r5-audit-fix` (+5).
- Fleet: 24 workers / 13 types healthy on ONE bundle `3b8b06f49864`, `/ready` true (embedder + reranker). Running code = committed
  code (`6f18017`: S4 + S8 installed, both flags off).
