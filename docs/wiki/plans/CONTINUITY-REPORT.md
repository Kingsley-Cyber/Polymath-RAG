...
**Next Action** — LATENT-QUERY-FUSION-V2 (plan-of-record `docs/wiki/plans/LATENT-QUERY-FUSION-V2.md`) on worktree `pmv4-fusion` (`831a2e5`). **DONE + PROVEN: F1 (11.328, `a6dc4a9`)** — pure `shared/polymath_shared/ranked_lane.py` + flag-gated capture + `trace['ranked_lanes']` in `retrieve_candidates`; UNIT+WORKTREE_INTEGRATION (51/51), flag-off byte-identical (`on_union==off_union`), NO selection change. **F2 (11.329, `831a2e5`)** — pure `shared/polymath_shared/ranked_fusion.py`: `fuse_ranked_lanes` = weighted RRF (`Σ weight(lineage_class)/(k+local_rank)`) + bounded local-winner preservation; UNIT_PROVEN (8/8) incl. a bridge's local rank-0 surviving a cap of 3 (truncated at preserve_top_n=0); candidate ordering only, no seating. **Next Action** — **OWNER DECISION on V2 (registers 11.327–334 done).** F1→F4 implemented + deployed + SAFE (sentinel), but the LEVEL-3 fixed-upstream causal replay (11.334) returned a NULL: given identical inputs, the fusion policy nets zero on deep-family admission (1 gain / 1 loss per mode, both HYBRID+WILDCARD), so the A/B improvement is confounded, not caused by fusion. V2 is currently LEFT LIVE per prior owner instruction. **The open decision is the owner's:** (a) KEEP V2 live (safe, mechanism works occasionally, product testing may reveal benefits offline metrics miss, fully reversible) — nothing further to do; or (b) REVERT (set `POLYMATH_CHAT_LATENT_FUSION`/`LATENT_SELECTION`/`BRIDGE_COMPILER`=0 in `.env` + port-gated bounce → V2 to a proven, flagged-off capability). A larger benchmark would NOT change the causal conclusion (control problem, not sample-size). Non-blocking: prune `pmv4-fusion`; do NOT tune `POLYMATH_FUSION_*` on the 10 probes. Standing: WLK1/WLK3; restore stash `49f57e0c`. **Do NOT re-run/reopen WLK2C/C7 or the F1–F3 proofs.**
