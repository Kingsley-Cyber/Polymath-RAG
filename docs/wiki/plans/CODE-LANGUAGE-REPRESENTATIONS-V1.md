...
status: "PLAN — the per-language representation contract that CODE-KNOWLEDGE-V1 slices C1–C7 and C13 implement. No code yet."
owner: "@king"
...
| 3. Graph | symbols and links; every link carries `resolution` (resolved / unresolved / ambiguous / external), `confidence`, `provenance` | parsers + analyzers, deterministic, never an LLM | `code_symbols`, `code_edges` (Postgres authority; Neo4j projection later) |
...
| **Meaning (class / parent)** | responsibility; the state it owns and its invariants; collaborators (from the graph); patterns; failure modes (retries, timeouts, race conditions); questions it answers |
| **Search vocabulary** | split identifiers, qualified names (`pkg.mod.Class.method`), route paths, decorator names |
| **Validation** | parse, `py_compile`, then ruff / pyright / mypy and the target tests where the repository configures them |
...
| **Relations** | `CONTAINS` (path hierarchy); `ALIASES` (alias → anchor); `DEPENDS_ON` (Compose `depends_on`, Actions `needs`); `REFERENCES_CODE` (a value equal to a code symbol, module path or file path in the same corpus, e.g. `handler: app.main:run`, `script: src/Server/AI.luau`: exact match, confidence-tagged); `REFERENCES_ENV` (`${VAR}`) |
| **Attributes** | dialect, document index, value type |
| **Meaning (file)** | what the config controls; which system reads it |
| **Meaning (section)** | what the settings do; defaults and allowed values; which code reads them (from `REFERENCES_CODE` / the reverse `REFERENCES_CONFIG`); risks (secrets placeholders, environment differences) |
...
| **Relations** | `CONTAINS`; `REFERENCES_CODE` (e.g. `[project.scripts] name = "pkg.module:func"` → that function); `DEPENDS_ON` (a dependency list → package names as `external` targets) |
...
| **Meaning** | what project / tool the file configures; what each table controls |
...
| **Meaning (class / parent)** | the game mechanic it implements; the state it owns; authority and replication assumptions (server-authoritative? trusts client input?); patterns (state machine, observer, object pool, component); failure modes (unvalidated remote input, race conditions on remotes, yields in hot paths, memory leaks from unclosed connections); questions it answers; game-design search phrases ("healthbar", "damage", "respawn", "loot table") |
| **Search vocabulary** | split `PascalCase` / `camelCase`, instance paths, remote and service names |
| **Validation** | `luau-analyze`; selene where configured; project tests (TestEZ / Jest-Lua) where present |
...
| **Tooling** | two passes (the owner's second note): (1) the YAML structure through the YAML card's parser; (2) each formula through Microsoft.PowerFx.Core (parse + bind + types) in a small .NET sidecar. .NET is installed only with the owner's OK. The export format is confirmed on the owner's app in C0 |
...
| From | To | How |
|---|---|---|
| YAML / TOML value | Python / Luau symbol or file | exact match of a module path, `module:function`, a qualified name or a file path in the same corpus (`REFERENCES_CODE`) |
| Python string literal | YAML / TOML key or env var | exact match (`REFERENCES_CONFIG`, low confidence) |
| Luau client script | Luau server script | the same remote path on both sides (`FIRES_REMOTE` ↔ `HANDLES_REMOTE`) |
| Power Fx formula | Power Automate flow | the binder's flow call (`CALLS_FLOW`) |

These live as `code_edges` rows in V1. The Neo4j projection and the doc↔code name links come later (decisions 5 and 11).
...
| C2 | the controlled vocabularies of §1 (symbol kinds, relations, attributes) in `code_symbols` / `code_edges` |
