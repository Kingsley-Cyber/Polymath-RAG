...
| 11.46 | SILICONFLOW-DECOMPOSITION + CANARY-LIMITER-ISOLATION + CANARY-CALL-DECOMP (owner 2026-09-02: "150 s per call — something ain't right"): streaming replay of the client's exact payload shows SiliconFlow serverless = ~30 s queue + 16–18 tok/s + degenerate output to the 2,500 cap, while the same Qwen2.5-7B weights on OpenRouter answer in 4–10 s (TTFT 0.5 s, 53–55 tok/s, valid JSON) → endpoint DROPPED, model kept in evaluation. The canary had shared production's PRIMARY AIMD row (`llm_cloud[default]`, 23 decreases) — now keyed `canary:<host>`; every canary call prints limiter wait / attempts / tokens / tok/s / finish_reason. | DONE (2026-09-02, work-log 2026-09-02-siliconflow-decomposition.md) |
