...
| D15 | **Canonical chunker = the TIER contract** (amended 2026-08-31, owner-ratified, EXECUTED as `tier_v3` / chunk-structure-v3): heading-bounded parents ~850 w target / 1,400 w max carrying REAL section text, tables/code atomic, noise via v4's region_role (fed real heading_path), <15-word stub drop. Implemented NATIVELY on v4 structural regions, NOT as a port of the v3.3 module — that module rewrites text (markup scrub, token re-joins) and would break v4's §8 offset contract (byte-exact substrings) that UI provenance and projection verifiers rely on. Page-scaffold headings ("Page N") merge to budget under the v3.3 OCR-lane rule. The interim chunkers (legacy_v1 fan-out groups; semantic_chunker) are retired for new ingests. Enrichment compiled ONE canonical parent per call until 2026-09-01; owner-blessed ENRICH-MICROBATCH-V1 amends this: 6-8 ITEM-ISOLATED parents per call (per-item validation = the same per-parent gate; split ladder 8→4→2→1 falls back to the single-parent path; partial acceptance — a bad item never re-buys batchmates). The per-parent artifact contract is UNCHANGED. | owner 2026-08-30; amended+executed 2026-08-31 (TIER-CHUNKER-V3, register 11.24) |
...
| Enrichment | ~~same model~~ SUPERSEDED (owner 2026-08-30): DEDICATED `nvidia` pool endpoint via `select_endpoint_for_stage("parent_enrichment", …)` (STAGE-PIN-V1), model `nvidia/nemotron-3.5-lightning-30b-a3b`, thinking OFF (`reasoning_effort:"none"` — LIVE-VERIFIED: baseline burns the whole budget thinking, "none" → finish:stop clean JSON), temp 0, JSON mode ON, output bounded (`max_tokens` 700 qual / 900 prod), limiter seed 36 RPM / conc_cap 4 PER ACCOUNT — CROSS-PROVIDER GROUP (owner 2026-08-30, supersedes the dual-nvidia group): pin group `["nvidia","groq5"]` — one NVIDIA + one dedicated Groq lane, each an independent AIMD bucket, docs sharded deterministically; failover rings within the group, so a whole-provider outage never stalls enrichment. groq5 runs qwen3.8-27b: Phase B must canary the enrichment schema on BOTH members. Never rerouted outside the group; one dark = reduced capacity (logged), all dark = loud fail. Phase B qualification must canary the parent-enrichment-v1 schema on THIS model (extraction-schema canary parsed clean but yielded 0 entities — different schema, different behavior) | owner directive 2026-08-30 + Part 3 budget |
...
### Phase 0 (scheduled separately) — canonical chunker swap — **EXECUTED 2026-08-31**
Adopt the tier contract (D15 as amended) as the v4 chunker: new `chunk_contract_version` (chunk-structure-v3), re-ingest, all projections rebuilt from the raw documents (everything derived is rebuildable). Enrichment units then equal canonical parents. Not a prerequisite for phases A–E (the compiler takes any parent; the input ceiling guards oversized ones). Executed as TIER-CHUNKER-V3 (work-log 2026-08-31-tier-chunker-v3.md, register 11.24): native implementation, generation purge at re-ingest, re-ingest driven by the "chunker" contract-key drift through reconciliation.
