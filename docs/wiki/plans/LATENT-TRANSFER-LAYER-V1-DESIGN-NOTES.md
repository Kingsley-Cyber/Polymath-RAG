...
## Part 3 — owner message 2026-08-30 (supersedes Parts 1–2 where they differ)

### Mode ownership (verified against `retrieval_modes.py:19-56`, `hybrid.py:152`, `graph.py:71`)
- **FAST = unchanged.** No latent search, no extra Qdrant queries, no
  behaviour change — the speed/control baseline.
- **HYBRID = primary consumer.** `FAST + lexical + OPTIONAL LATENT RESCUE`.
  Rescue/augmentation, NOT extra RRF votes ("the generated layer must not
  get four votes and overpower real retrieval"). Contract:
  `candidates = baseline_candidates ∪ latent_rescue_candidates`; latent may
  introduce parents ordinary retrieval missed; it never replaces or
  suppresses ordinary candidates; the existing child reranker adjudicates
  real source text.
- **GRAPH inherits** enriched HYBRID evidence, then adds the canonical
  graph. Never `LLM mechanism → Neo4j`; always `mechanism → parent →
  original children → canonical graph`.

### LLM job shrunk to six outputs (Polymath already extracts FACT/PROCEDURE/CONCEPT — no duplicate "full extraction")
| output | purpose | budget |
|---|---|---|
| summary | faithful parent overview | 80–120 words |
| children[].gist | every child represented | 20–35 words/child |
| abstraction | domain-neutral principle | 25–45 words |
| mechanisms | how/why it works | max 2 |
| affordances | what problems it helps solve | max 2 |
| questions | user-language retrieval bridges | max 3 |
Lean output JSON: `{summary, children:[{id, gist}], abstraction,
mechanisms:[str], affordances:[str], questions:[str]}`. NOTHING the runtime
already knows is generated by the model (no ids/hashes/provider/coverage
lists). The worker attaches: `enrichment_id, parent_id, doc_id, corpus_id,
source_child_ids, input_hash, contract, provider, model, prompt_version`.
Rule: **never ask the LLM to generate information Polymath already knows
deterministically.**

### Input stripped
Send only `PARENT_ID` + per child `CHILD <ordinal> | <child_id>` +
(context metadata only if useful) + the child text. Do NOT send the old
parent summary (anchors the model to a lossy centroid), document profile,
corpus map, canonical entities, facts, provenance envelopes, receipts,
embedding info, hashes, provider config, Qdrant/Neo4j metadata.
Verified: `semantic_chunker.py:41-42` `parent_fanout: 4`,
`child_target_chars_fallback: 1200`; `:383 _with_parents` builds the parent
row's `text` from `summarize_children(...)` (an extractive centroid, not
source text). Typical request ≈ 4 × 1,200 chars ≈ 1.1–1.6K input tokens +
≈ 0.45–0.7K output ≈ 2K tokens/parent. Oversized single sentences are legal
under the chunk contract → the worker enforces a real model-token input
ceiling, never assumes ≤1,200 chars.

### Coverage validation is free
Require exactly one gist per input child; worker checks
`returned_ids == expected_ids` (missing / unknown / duplicate → REJECT).

### Only TWO vectors per enriched parent (initially)
- `latent_abstraction` — "what general idea is this?" (cross-domain match)
- `latent_transfer` — "what does this knowledge do / how could it help?"
  = mechanisms + affordances rendered as one text
Keep `children[].gist`, `questions`, and the full result in Postgres;
do NOT vectorize them initially; measure whether question vectors add
unique relevant hits before adding a third kind. The normal parent/section
summary lane already answers "what is this passage about?" — no extra
summary vector.

### Query time
Same `qvec` as the neural lanes (no re-embed); two extra filtered
nearest-neighbour searches (`representation_kind=latent_abstraction`,
`=latent_transfer`). No LLM, no query-time mechanism/question generation.
Caps: `abstraction_top_k = 8`, `transfer_top_k = 8`, dedupe by
`parent_id`, `max_latent_parents = 2–3` into the child-deepening pool →
up to ~3 children each → reranker. "This prevents the system from becoming
an analogy generator instead of a retrieval system."

### Process (owner)
Implementation ONLY after the owner personally validates the base RAG
pipeline e2e (UI, MCP, query time, ingestion, extraction). Then a
code-base mapping with notes/citations/references of where and what to do,
then implement. Base is NOT yet validated.

### Code facts found while checking Part 3 (must shape the plan)
- HYBRID's lexical parents are appended to `sections` AFTER pass-1 ran
  (`hybrid.py:~235-250`), so they are NOT deepened; lexical rescue admits
  the lexical CHILD hits themselves (`rescue_lexical`). Latent rescue
  therefore needs an explicit deepening step: per latent parent, one
  `routing_search(..., {representation_kind: routing_child, corpus_id,
  parent_id})[:max_children_per_section]` (the same primitive pass-1 uses).
