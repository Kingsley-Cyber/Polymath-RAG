...
|---|---|---|---|---|
| P1.c judge + composition | `p1c_patch.py` (composer slots 8/4/3/3, sigmoid judge scale, bounded agreement, `final_detail`, receipts, baseline metrics), `p1c_patch2.py` (rerank prefix 20 → 28, measured reason), `p1c_worklog_skeleton.md`, `p1c_runs.sh` | `.venv/bin/python docs/wiki/plans/handoff/p1c_patch.py && .venv/bin/python docs/wiki/plans/handoff/p1c_patch2.py` | `pytest tests/determinism/test_candidate_engine.py` (4 new composer tests) | baseline = `chat-baseline-p1b-B-after` (survival 22 / 27 = 0.815, MRR 0.516); run `p1c_runs.sh` (B deterministic, M, B with the LLM for citation precision vs P0.d's 1.000) |
