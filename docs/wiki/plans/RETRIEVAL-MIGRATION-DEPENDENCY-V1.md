...
| S12 | Existing corpus backfill | repo S14 | **FIRST CORPUS COMPLETE (d7-h1-test); cinema PARTIAL** | owner authorized backfill (2026-09-08). **`d7-h1-test` = VNEXT_COMPLETE 2026-09-08 (11.176)** — the whole data-regen chain driven end-to-end on a real corpus: 78/78 eligible parents mapped (0 unresolved), 3/3 vNext profiles, 30 atoms across all 10 kinds (reconciled 30==30). Capacity canary passed first (26/26 parents, 0 err, spread across 3 Groq accounts via CONCURRENCY-SPREAD-V1); sanctioned map/profile lanes only, no paid fallback. This is the first proof the migration substrate reaches its terminal readiness verdict autonomously. **cinema: profile scale DONE** — 67/67 re-profiled under vNext + 609 atoms all-kinds, reconciled (11.169); **parent-MAP scale PARTIAL** — 576/11,993 eligible parents mapped (55/67 docs still 0-mapped), `parent_map_backfill.py --corpus cinema --project --concurrency 6` resumable/idempotent, RPD-paced multi-session campaign (~11,417 parents remain). No cutover before coverage + canary gates. **Backfill finding 2026-09-08:** a full run advanced 551→576 (+25); its ~657 routed calls concentrated on `map_groq1` while 55 fresh docs yielded 0 maps with 0 errors. The router spread is verified healthy in isolation (60 concurrent → 10/account; live `route()` returns clean 6-way spread, 0 no-capacity), so the concentration did NOT reproduce out of the concurrent run — investigate the backfill `infer` fail-open (`route()`→None ⇒ `next(iter(eps))`=map_groq1) + live-snapshot timing under `--concurrency` before the next large campaign run. Not a blocker; cinema stays multi-session regardless. |
