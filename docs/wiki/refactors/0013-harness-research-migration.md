...
| R1 | contracts: HARNESS_ACTION in the closed vocabulary; `harness_action`, `harness_receipt`, `hypothesis_state`, `hypothesis_transition`, `evidence_admission` schemas + examples; step/submission/result extensions; neutrality with source names | DONE | 11.266 — 61 adapter tests green |
| R2 | substrate: migration 0062 (hypotheses, transitions, harness actions, admitted evidence); pure hypothesis state machine; HARNESS_ACTION issue/pause/receipt; admitted-only context; θ/φ ledger hooks; `.env.example` names | DONE | 11.267 — 72 adapter tests green; fleet bounce at merge |
...
| R4 | `trail.product_discovery` 2.0.0 on the new loop; Trail-owned acquisition executors removed; bounded Trail operations; retired-paths guard; dead-reference audit | DONE | 11.268 — 78 adapter tests green; live run stops honestly at `D_project` (HR3 planned) |
