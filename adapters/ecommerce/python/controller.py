...
def cmd_init(args):
    ...
    if getattr(args, "document_id", None):  # docs/26 §8: document scope for the corpus lanes (retrieve + plan; never an unscoped evidence call)
    ...
    for name in ("init", "status", "submit", "step", "context-export", "handoff",
        ...
        if name == "init":
            ...
            sp.add_argument("--document-id", action="append", default=None, dest="document_id",
                            help="docs/26 §8: restrict the corpus lanes to these document ids (repeatable); the evidence route is never called unscoped while a scope is active")
        if name == "submit":
