...
`limit`). The evidence route cannot be scoped to documents, so while a scope is
active the adapter never calls it: rows come from retrieve/plan only and θ
interprets locally (`corpus_backend.evidence_skipped` records the decision;
before v2.2.0 the skipped route was the answer route and the key `chat_skipped`). A backend that does not
