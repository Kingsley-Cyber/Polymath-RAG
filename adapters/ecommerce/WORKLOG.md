...
## 2026-09-04 — v2.1.2: final correctness pass (verified defect → owning file → failing regression → fix)

1. **Population canary split** (`tests/calibration_acceptance.py`): `latent_population_discovery` is gone. `open_field_population_discovery` (mandatory) counts OPEN_FIELD leads instantiated outside the seed; `latent_population_resolution` requires a LATENT lead whose `latent_structure_id` is an admitted structure, status INSTANTIATED, and admitted field records whose `lead_id` points back — OPEN_FIELD never satisfies it. Advisory in STANDARD, mandatory in SOURCE_AGNOSTIC_CALIBRATION (`policies.calibration.advisory` / `source_agnostic_mandatory`). The books run had 6 LATENT leads nominated, 0 instantiated, and still "passed" the old canary — that proved nothing.
2. **Heterogeneous-source canary** measures GENERATION and only in the dedicated mode: `--calibration-mode SOURCE_AGNOSTIC_CALIBRATION` (explicit, never inferred from `--heterogeneous-docs`). STANDARD reports NOT_EVALUATED — any source MAY generate, no source MUST. PASS = a configured heterogeneous row classified relevant fed a latent structure or a hypothesis hop; the hypothesis may die.
3. **Corpus-wide presence audit** (`provenance.corpus_presence`, `corpus_polymath.py --presence`): one CorpusPresenceReceipt per final concept (exact phrase / normalized multi-token / observed product / example hits, document hits, documents_checked, method_version) from existing calls only (GET /documents, POST /retrieve per concept phrase). `lineage` and the acceptance test consume it; presence changes naming only, never demand.
4. **Field origin** (`provenance.field_origin`): FIELD_NAMED / WORKAROUND_DERIVED / NOT_FIELD_ORIGINATED with a receipt (matched records, terms, explicit mentions, workaround refs); only records with valid field provenance count; one generic shared token never establishes lineage. `field_originated = origin ∈ {FIELD_NAMED, WORKAROUND_DERIVED} AND not corpus-named`. Books effect, recorded not optimized: the keychain fob drops to NOT_FIELD_ORIGINATED (it had been carried by the single token "dose"); the other four keep their origin (2 WORKAROUND_DERIVED, 2 FIELD_NAMED).
5. **Document scope fails closed** (`corpus_polymath.py`): no advertised `document_ids` → `capability_failure {document_scoped_corpus_retrieval, BLOCKED_CAPABILITY_UNAVAILABLE}`, no request leaves the process, `--generic` does not bypass; the controller records the coverage deficit.
6. **Receipt terminology**: `legitimate_echo_survival` → `legitimate_corpus_overlap_survival` (it triggers on corpus_named OR example overlap; both counts exposed); every concept receipt carries `corpus_named`, `corpus_example_overlap`, `field_origin`; utilization and the HTML report show field origins, corpus-named and presence-audited counts.
7. **Novel metadata** (data provenance, not content): `polymath:ecom-meta-v1` document `doc_d444fe46…` ("Alchemy (Sutherland)", 508,301 normalized bytes, 1,514 chunks) was the romance novel *Always Alchemy* by Elodie Hart (© 2024 Sara Madderson). The documents API has no PATCH: the spooled source bytes were verified (normalized length equal, retrieved chunks verbatim), the mislabeled ingestion deleted, and the SAME body re-ingested as `Always_Alchemy_Hart.md` with corrected frontmatter (title, author, genre, note). Fiction stays in the corpus on purpose — it is the source-agnostic test.
Harness: 537 checks (section 23, each defect failing-then-passing; sections 20i/21 assertions updated to the corrected law). `scripts/repo_guard.py` now ignores the git-ignored local evidence ledger like the other research runtime artifacts.

**Item 8 — novel-seeded SOURCE_AGNOSTIC calibration (run `calib_novel_02`, receipt `docs/calibration/2026-09-04-novel-run-02.md`):** seed = the toddler-breakfast-before-first-nursery-drop-off scene of *Always Alchemy*, corpus scoped to the novel (policy B), no population supplied. 62 novel rows → 6 relevant + 56 IRRELEVANT → 6 typed latent structures → 6 LATENT population leads (structure language only) → scout resolved 15 real communities (source_lane LATENT) → VOI queue instantiated r/daddit (12 records, 4 voices), r/beyondthebump (12, 3), r/Fosterparents (6, 3) → 3 ANCHOR clusters → 6 FIELD_ANCHORED situations → 4 bridges → L4 fresh evaluator (2 REVISE, 2 REJECT) → 2 research rounds, 39 observations, 13 contradicting → every bridge REJECTED, two of them field-caused → 1 PROPOSED mechanism, 0 SUPPORTED → NO_DEFENSIBLE_BRIDGE (Law 12). Canaries: heterogeneous_source_reasoning PASS, latent_population_resolution PASS, irrelevant_source_rejection PASS (trap: the club scene), hypothesis_death PASS; corpus_independence / field_originated_opportunity / open_field_population_discovery NOT_TRIGGERED (zero products; no OPEN_FIELD nomination) — named in `not_triggered_mandatory`. What the field volunteered (mess-hiding adult garments) is recorded as a direction, not built.

**Live defects found by run 02 (each: failing regression → fix → passing; harness section 23g–23l):** hop_refs refused admitted latent-structure / corpus-observation ids (controller known-id set); the L4 dossier carried no field-record or latent-structure summaries (evaluator.build_dossier, `lane`); every bridge REJECTED with budget left stalled at `gaps` (transitions.evidence_sufficient); an EMPTY submitted output list read as "not submitted" and the envelope declared all-REJECTED hypotheses missing (controller step `_submitted_here`; context `all_rejected`); the `no_defensible_bridge` edge reached the terminal without its verdict — the persisted label of run 02 is `STOPPED_WITHOUT_QUALIFICATION` because terminal runs are immutable (controller advance sets NO_DEFENSIBLE_BRIDGE now); `lived_world.summary` counted only `search_mode` LATENT leads; zero-product runs FAILED the concept canaries (acceptance: NOT_TRIGGERED with reason). Section-12 sufficiency assertion corrected (it encoded the stall as correct). **Books regression** (`docs/calibration/2026-09-04-books-run-01-regression.md`): pass, no semantic regression; keychain fob → NOT_FIELD_ORIGINATED under the stronger law; presence audit: none of the five concepts is named anywhere in the 10-document corpus. Harness: 555 checks. Doctor: ok.


---

## 2026-09-20 — v2.2.0: the corpus lane asks for EVIDENCE, never for an answer (GOVERNED-CONVERGENCE-V1 TG3)

**Why.** The owner's evidence-boundary rule (2026-09-19/20): an agent gets corpus knowledge as evidence and reasons
ONCE. v2.1.x did the opposite on its default lane — every compiled reformulation went to Polymath's answer route and
the synthesized replies were stored as `corpus_answers`: a second reasoner nested inside θ's reasoning (5 synthesis
calls per corpus per visit; most of them abstentions). This was the last real nested-synthesis caller. Polymath now
serves the right thing: `POST /chat/evidence` → `EvidencePacket` (full planning + Corpus Explore, roles, CA4 grades,
provenance, `synthesis_performed: false`).

**What changed (owning file → change).**
1. `python/corpus_polymath.py` — DELETED `ask_corpus`, `chat_question`, `answer_record`. NEW `explore_corpus()` (exactly
   `{message, corpus_id, mode: WILDCARD, corpus_explorer: true}`), `packet_errors()` (FAIL CLOSED: version, synthesis
   flag, `schemas/evidence_packet.json`), `rows_from_packet()` (text, utility_role, ca4_grade, c4_valid, origin, lineage,
   provenance, packet_query_ids; id space shared with the retrieve lanes), `packet_record()`, `_merge_row()` (one row per
   chunk across lanes; the fuller passage wins). `--via evidence|plan` (default `evidence`, keyed on
   `capabilities.contracts["evidence-packet"]`); the retired `--via chat` is refused (exit 2). Node `corpus` = ONE call
   per corpus with the run's ORIGINAL signal — never the 3–5 reformulations; node `corpus_mechanisms` = one call per
   compiled question, capped (`--max-evidence-calls`, default 12), the skip recorded. A contract mismatch →
   `capability_failure{corpus_evidence_packet, EVIDENCE_CONTRACT_MISMATCH}` and NO further request (never a fallback);
   an unreachable evidence route is a recorded error and the row lanes still run. `_post` refuses any path outside
   `{/chat/evidence, /retrieve, /retrieve/plan}` BEFORE I/O. Every request is counted and timed (`corpus_backend.calls`,
   `.timings_ms`, `polymath_chat_calls`, `polymath_evidence_calls`) and carries an attributable User-Agent
   (`opportunity-research/2.2.0 corpus_polymath run:<id> node:<node>`), so Polymath's own receipt ledger can prove it.
   Document scope (policy B): the evidence route cannot be scoped, so it is skipped — key `evidence_skipped`.
2. NEW `schemas/evidence_packet.json` — the skill-dialect (schema-lite: no const / $ref) rendering of polymath-v4
   `contracts/evidence/v1/evidence_packet.schema.json`, pinned to its sha256; the harness fails on drift wherever the
   two repos sit side by side.
3. Payload / state: `corpus_answers` → `corpus_packets` (`authority: CORPUS_EVIDENCE_PACKET`; no `answer`, no
   `abstained` — `n_evidence` + the CA4 grade distribution instead). `graph/control_graph.yaml` (corpus,
   primitives context, corpus_mechanisms), `models.py`, `memory.py` (`CORPUS_EVIDENCE_PACKET` node type),
   `context.py` (priority 3), `lived_world.py` (`authority_of_answer`).
4. Readers tolerate LEGACY state: `utilization.py` (packets / packet rows / grade + role distributions / evidence vs
   synthesis calls; legacy answers counted separately), `report.py` ("Corpus evidence packets"; legacy answers render
   as "legacy synthesis, not evidence"), `provenance.py` (a packet RETRIEVES rows — being in a packet is not a citation).
5. Docs: `docs/22` rewritten around the evidence boundary (+ §4 the request ledger), `docs/18` §10, `docs/26` §8,
   `SKILL.md` (corpus + corpus_mechanisms rows, the Polymath section: YOU are the only reasoner).

**Why ONE call and not one per reformulation:** Polymath owns retrieval planning (its compiler + Corpus Explore do the
expansion, grounded in the corpus). Sending our own rewordings would re-introduce caller-side decomposition the
boundary forbids. The EXPLORE plan rows still ride along for breadth — that lane is retrieval, not reasoning.

**Why fail closed and not fall back:** a reply of another version, or one that reports a synthesis, means the boundary
itself is not what we think it is; consuming part of it, or quietly using another lane, would hide exactly the defect
this migration exists to remove.

**Harness:** 580 checks (555 → 580; §17 rewritten around the evidence boundary — the stub still SERVES the synthesis
routes so a call would be seen, not 404'd; §22g / §23f extended: no unscoped evidence or synthesis call under a
document scope). `controller.py doctor` green. Negative control on one stub: v2.1.2 → 5 × `POST /chat`; v2.2.0 → 0.

**Live acceptance (receipt `docs/calibration/2026-09-20-tg3-evidence-lane-acceptance.md`):** Claude-controlled run
against live Polymath, corpus `cinema`: 1 evidence call, 0 synthesis calls (adapter ledger AND Polymath's receipt
ledger), 15 packet rows with role / grade / provenance, primitives and hypothesize accepted on the first submit,
HTML report rendered by the existing renderer. Findings: packets carry 240-char text previews (Polymath-side, not
fixed here); Corpus Explore did not fire on the signal (`PLAN_FALLBACK`).

**NOT done here (deliberately):** no mirror to the deployed Hermes copy (`~/.hermes/standalone/opportunity-research`
stays v2.1.2 and still calls the answer route until the TG4 mirror); no push; `MIRROR_RECEIPT.json` untouched (its
reference tree no longer exists — TG4 repoints `tests/mirror_check.py`). Rollback: `--via plan`, or `git revert`.


---

## 2026-09-20 — v2.3.0: the governed entrypoint — HarnessActionV1 → this skill's tools → HarnessResearchReceiptV1 (GOVERNED-CONVERGENCE-V1 TG4)

**Why.** Two product-research systems existed and never touched: this skill (real-world, ungoverned, its own
`evidence_score`) and Polymath's governed adapter → TrailSignal path (lawful, never driven by a real harness). The owner's
decision: move this skill's useful behaviour BEHIND the governed path — reuse the acquisition tooling as the harness and
the renderer as the dossier; build no second harness and no second report system; TrailSignal stays the only judge.

**What was genuinely missing (and is now here).**
1. NEW `python/adapter_receipt.py` — `build_receipt(action, observations | field_records | supplier_candidates, harness_id,
   started_at, completed_at, tool_trace)` → a `HarnessResearchReceiptV1` the adapter's `adapter_submit kind=receipt`
   accepts. ONE static role table (18 skill roles → TrailSignal's 11; MECHANISM_SUPPORT / INSIDER_LANGUAGE map to nothing and
   such items are omitted, never coerced; `contradicts: true` always wins); source class = registered domain (from
   TrailSignal's `source_capabilities.csv`) else the item's own platform / family, else UNKNOWN → omitted; suppliers reuse
   `executors._parse_price` / `_parse_moq` (price → role `price` + metric, minimum order → role `supply` + metric; a policy
   default MOQ is never an observed number); bounds (excerpt 600 / claim 2000 / context 2000; sources + observations clamp
   to the ACTION's budget; a blown query budget is RECORDED, never rewritten); hypothesis ids ⊆ the action's; tool trace
   bound to the action's search intents. Deterministic. NO score: whitelist construction + `validate_receipt` refuses any
   key matching `score|rank|weight`.
2. **Harvest provenance** — `retrieved_at` + `published_at_if_known` (explicit `null` = the page shows no date) are now
   optional fields of `observation` / `field_record` / `supplier_candidate` and are taught in `prompts/evidence_judgment.md`
   and `prompts/community_instantiate.md`. The builder OMITS anything without them and says so in the receipt's
   `limitations`: a timestamp is never back-filled with "now", a known publish date is never dropped, prior-run records
   and corpus rows are never exported as fresh field evidence.
3. NEW `schemas/harness_receipt.schema.json` — BYTE COPY of polymath-v4 `contracts/adapter/v1/harness_receipt.schema.json`,
   sha256 pinned in the module; validated with a small JSON-Schema subset reader (the skill's schema-lite `required` means
   non-empty and would reject the contract's lawful nulls).
4. NEW `python/governed_run.py` — the append-only run JOURNAL (`state/<run_id>.governed.json`): every issued step WITH its
   readable evidence (recorded once however often polled), every submission + the adapter's answer (a rejection stays a
   rejection), every receipt + the builder's omission report, the final `AdapterResultV1`. It drives nothing, calls nothing.
5. `python/report.py` — `build_model_from_governed(journal)` feeds the EXISTING `render`: TrailSignal's record VERBATIM
   (score, confidence, subscores, coverage gaps, versions; refusals with reason codes), hypotheses as the adapter's ledger
   holds them, field observations split admitted / rejected-WITH-reason-code and joined to the receipt for quote + URL +
   metric, admitted supplier observations as lead cards (no `evidence_score` — it never appears in a governed dossier),
   boundary steps as "Corpus evidence packets", limitations + unknowns in Unresolved, lineage in the audit. Standalone
   reports are byte-for-byte unchanged in structure (pinned).
6. `SKILL.md` — new top section "Governed entrypoint"; `docs/27_governed_entrypoint.md`; `tests/mirror_check.py` repointed
   (REFERENCE = this git repo, DEPLOYED = `~/.hermes/standalone/opportunity-research`).
7. Carried from polymath-v4 RB5 (same day): `schemas/evidence_packet.json` re-pinned (the packet now carries a bounded
   verbatim excerpt + `text_truncated` / `text_chars`); `rows_from_packet` passes the indicator through.

**Why omission instead of repair:** every one of those gaps (no timestamp, no publish-date statement, unknown source kind,
vocabulary-only role) is a fact about how the harvest was done. Filling it in would make the receipt say something the
harness never observed — and TrailSignal's freshness and suitability gates would then judge an invented fact.

**Why the dossier never shows `evidence_score`:** LAW 1 — the only score in a governed run is TrailSignal's. Showing a
second number beside it, however labelled, invites exactly the blending the governed path exists to prevent.

**Harness:** 609 checks (580 → 609; section 24). Cross-repo, wherever polymath-v4 sits beside this repo: the receipt schema
is a byte copy; the built receipt is structurally equal to Polymath's acceptance fixtures; and Polymath's OWN
`transitions.validate_receipt` — the exact check `adapter_submit` runs — accepts it and refuses it under another action id.
Real-data check (not committed — it holds corpus text): a journal rebuilt from the live R1 governed run (14 answered steps,
5 receipts, 33 admitted observations, 1 score + 1 refusal) renders `GOVERNED — TRAIL SCORED` with no evidence score.

**NOT proven here (deliberately):** no live governed run drove this code — `adapter_submit` with a skill-built receipt, and
